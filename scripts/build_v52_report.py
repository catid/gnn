...
SUMMARY_PATH = REPORTS / "summary_metrics_v52.json"
REPORT_PATH = REPORTS / "final_report_v52_transfer_specialist_recheck.md"
...
def phase_records(phase: str) -> list[dict[str, Any]]:
    ...
    for selector in SELECTORS:
        prefix = f"v52-{spec['regime']}-{selector}-32-xl"
...
def main() -> None:
    ...
    REPORT_PATH.write_text(
        "\n".join(
            [
                "# APSGNN v52: Transfer-Specialist Recheck",
                "",
                "## What Changed",
                "",
                "v52 reruns the only regimes that had historically favored StageLate-0.5: T1, T1r, T2b, and T2c, using one more fresh matched same-seed XL matrix.",
                "",
                "## Recommendation",
                "",
                f"Current overall winner on the fresh v52 transfer/non-ingress slice: {SELECTORS[final_recommendation]['label']}.",
                "",
                f"- Summary JSON: [summary_metrics_v52.json]({SUMMARY_PATH})",
                f"- Report: [final_report_v52_transfer_specialist_recheck.md]({REPORT_PATH})",
                "",
            ]
        ),
        encoding="utf-8",

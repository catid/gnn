...
def latest_runs(regime: str, pair: str, schedule: str, tags: set[str] | None = None) -> list[Path]:
    ...
    for candidate in sorted(RUNS.glob(f"*-{PREFIX}-*")):
        ...
        if meta is None:
            continue
        if meta["regime"] != regime or meta["pair"] != pair or meta["schedule"] != schedule:
            continue
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run eval sweeps for v64 runs.")
    ...
    parser.add_argument("--settle-cap", type=int, default=24)
    ...
    for regime in args.regimes:
        for schedule in args.schedules:
            for pair in args.pairs:
                for run_dir in latest_runs(regime, pair, schedule, tag_set):

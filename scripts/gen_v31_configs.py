...
ARMS = {
    ...
    "visit_taskgrad_half_agree_mutate_z00": {
        "display": "VT-0.5 CAG-z0.0",
        "kind": "mutation",
        "growth": {
            "split_mode": "mutate",
    ...
    "visit_taskgrad_half_agree_mutate_z25": {
        "display": "VT-0.5 CAG-z0.25",
        "kind": "mutation",
        "growth": {
            "split_mode": "mutate",
            "mutation_stage_index_min": 6,
            "mutation_selected_fraction": 1.0,
            "mutation_score_margin": 0.75,
            "mutation_min_visit_z": 0.25,
            "mutation_min_query_grad_z": 0.25,
        },
    },
    "visit_taskgrad_half_agree_mutate_z50": {
        "display": "VT-0.5 CAG-z0.5",
        "kind": "mutation",
        "growth": {
            "split_mode": "mutate",
            "mutation_stage_index_min": 6,
            "mutation_selected_fraction": 1.0,
            "mutation_score_margin": 0.75,
            "mutation_min_visit_z": 0.5,
            "mutation_min_query_grad_z": 0.5,
        },
    },
}
...
def build_config(arm: str, regime: str, schedule: str) -> dict:
    ...
    return {
        ...
        "runtime": {
            "run_name": f"v31-{regime}-{arm}-32-{schedule}",
            "notes": f"selector={arm_spec['display']}",
        },
        "growth": {
    ...
    for arm in ARMS:
        for regime, schedules in regime_schedules.items():
            for schedule in schedules:
                config_name = f"v31_{regime}_{arm}_32_{schedule}.yaml"
                run_name = f"v31-{regime}-{arm}-32-{schedule}"
                dump_yaml(build_config(arm, regime, schedule), CONFIGS / config_name)
                write_script(
                    SCRIPTS / f"train_v31_{regime}_{arm}_32_{schedule}.sh",
                    config_name,
                    run_name,
                    False,
                )
                if schedule == "l" and regime != "t2a":
                    write_script(
                        SCRIPTS / f"smoke_v31_{regime}_{arm}_32_{schedule}.sh",
                        config_name,
                        run_name,
                        True,
                    )

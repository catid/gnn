...
def write_script(path: Path, config_name: str, run_name: str) -> None:
    ...
    path.write_text(
        f"#!/usr/bin/env bash\nCONFIG=configs/{config_name} DEFAULT_SEED=176234 DEFAULT_RUN_NAME={run_name} exec \"$(dirname \"$0\")/_train_wrapper_single_gpu.sh\"\n",
        encoding="utf-8",
    )
    path.chmod(0o755)


def build_config(arm: str) -> dict:
    return {
        "model": copy.deepcopy(MODEL_BASE),
        "task": {"name": "memory_growth", **copy.deepcopy(REGIME)},
        "train": {
...
def main() -> None:
    ...
    for arm in ARMS:
        config_name = f"v55_t2b_{arm}_32_xl.yaml"
        run_name = f"v55-t2b-{arm}-32-xl"
        dump_yaml(build_config(arm), CONFIGS / config_name)
        write_script(SCRIPTS / f"train_v55_t2b_{arm}_32_xl.sh", config_name, run_name)

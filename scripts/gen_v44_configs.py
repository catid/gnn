...
ARMS = {
    ...
    "stageadaptive_late_half_agree_mutate_z00_m075_f0125": {
        "display": "StageLate-0.5 CAG-z0.0-m0.75-f0.125",
        "growth": {
            "split_mode": "mutate",
    ...
    "stageadaptive_late_half_agree_mutate_z00_m075_f025": {
        "display": "StageLate-0.5 CAG-z0.0-m0.75-f0.25",
        "growth": {
            "split_mode": "mutate",
            "mutation_stage_index_min": 6,
...
def write_script(path: Path, config_name: str, run_name: str) -> None:
    ...
    path.write_text(
        f"#!/usr/bin/env bash\nCONFIG=configs/{config_name} DEFAULT_SEED=90234 DEFAULT_RUN_NAME={run_name} exec \"$(dirname \"$0\")/_train_wrapper_single_gpu.sh\"\n",
        encoding="utf-8",
    )
    path.chmod(0o755)
...
def build_config(arm: str, regime: str) -> dict:
    return {
        ...
        "runtime": {
            "run_name": f"v44-{regime}-{arm}-32-xl",
...
def main() -> None:
    ...
    for arm in ARMS:
        for regime in REGIMES:
            config_name = f"v44_{regime}_{arm}_32_xl.yaml"
            run_name = f"v44-{regime}-{arm}-32-xl"
            dump_yaml(build_config(arm, regime), CONFIGS / config_name)
            write_script(SCRIPTS / f"train_v44_{regime}_{arm}_32_xl.sh", config_name, run_name)

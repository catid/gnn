...
SUMMARY_PATH = REPORTS / "summary_metrics_v74.json"
REPORT_PATH = REPORTS / "final_report_v74_max_augmented_output_readout.md"
PACK_DEFS_PATH = REPORTS / "v74_pack_definitions.json"
BASE_LR = 2.0e-4
EXPECTED_TRAIN_STEPS = {"p": 300, "m": 900, "l": 1350}
RUN_RE = re.compile(
    r"v74-collision-(?P<regime>c[12])-(?P<condition>mean|maxaug)-(?P<pair>[a-z0-9_]+)-32-(?P<schedule>p|m|l)(?:-(?P<tag>(?!s\d+$)[^-]+))?(?:-s(?P<seed>\d+))?$"
)
...
def latest_runs() -> list[tuple[dict[str, str], Path]]:
    ...
    for candidate in sorted(RUNS.glob("*-v74-*")):
...
def summarize_run(run_dir: Path, meta: dict[str, str]) -> dict[str, Any]:
    ...
    record = {
        ...
        "max_scale": float(config["model"].get("cache_output_max_scale", 0.0)),
    }
...
def main() -> None:
    ...
    pilot_choices = {
        ...
        "maxaug": choose_pilot(records, "maxaug"),
    }
    ...
    for regime in ("c1", "c2"):
        ...
        maxaug_records = [r for r in main_records if r["regime"] == regime and r["condition"] == "maxaug"]
        main_summary[regime] = {
            "mean": group_summary(mean_records),
            "maxaug": group_summary(maxaug_records),
        }
        dense_gaps[regime] = (
            main_summary[regime]["maxaug"].get("dense_mean", {}).get("mean", 0.0)
    ...
    rerun_summary = {
        ...
        "maxaug": group_summary([r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "maxaug"]),
    }

    probe_audit: dict[str, dict[str, Any]] = {}
    for condition in ("mean", "maxaug"):
    ...
    positive = (
        ...
        and rerun_summary["maxaug"].get("dense_mean", {}).get("mean", 0.0)
        >= rerun_summary["mean"].get("dense_mean", {}).get("mean", 0.0)
    )

    summary = {
        "experiment": "v74_max_augmented_output_readout",
    ...
    report_lines = [
        ...
        f"- Max-augmented LR multiplier: `{pilot_choices['maxaug'].get('lr_multiplier', 0.0):.1f}`",
        "",
        "## Main Results",
        "",
        f"- `c1` mean dense `{main_summary['c1']['mean'].get('dense_mean', {}).get('mean', 0.0):.4f}`, maxaug dense `{main_summary['c1']['maxaug'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c1', 0.0):+.4f}`",
        f"- `c2` mean dense `{main_summary['c2']['mean'].get('dense_mean', {}).get('mean', 0.0):.4f}`, maxaug dense `{main_summary['c2']['maxaug'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c2', 0.0):+.4f}`",
        "",
        "## Fresh Rerun",
        "",
        f"- Regime: `{strongest_regime}`",
        f"- Mean dense: `{rerun_summary['mean'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        f"- Max-augmented dense: `{rerun_summary['maxaug'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        "",
        "## Probe Audit on C2 Best Checkpoints",
        "",
        f"- Mean sink/cache/cache-max/home probe test acc: `{probe_audit.get('mean', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('mean', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('mean', {}).get('home_cache_max_state', 0.0):.3f}` / `{probe_audit.get('mean', {}).get('home_hidden_state', 0.0):.3f}`",
        f"- Max-augmented sink/cache/cache-max/home probe test acc: `{probe_audit.get('maxaug', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('maxaug', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('maxaug', {}).get('home_cache_max_state', 0.0):.3f}` / `{probe_audit.get('maxaug', {}).get('home_hidden_state', 0.0):.3f}`",
        "",
        "## Conclusion",
        "",

...
SUMMARY_PATH = REPORTS / "summary_metrics_v67.json"
REPORT_PATH = REPORTS / "final_report_v67_cache_summary_fusion.md"
PACK_DEFS_PATH = REPORTS / "v67_pack_definitions.json"
BASE_LR = 2.0e-4
EXPECTED_TRAIN_STEPS = {"p": 300, "m": 900, "l": 1350}
RUN_RE = re.compile(
    r"v67-collision-(?P<regime>c[12])-(?P<condition>baseline|fusion)-(?P<pair>[a-z0-9_]+)-32-(?P<schedule>p|m|l)(?:-(?P<tag>(?!s\d+$)[^-]+))?(?:-s(?P<seed>\d+))?$"
)
...
def latest_runs() -> list[tuple[dict[str, str], Path]]:
    ...
    for candidate in sorted(RUNS.glob("*-v67-*")):
        ...
        if meta["schedule"] == "p":
            ...
            continue
        latest[(meta["regime"], meta["condition"], meta["pair"], meta["schedule"], meta["seed"])] = (meta, candidate)
...
def summarize_run(run_dir: Path, meta: dict[str, str]) -> dict[str, Any]:
    ...
    record = {
        ...
        "fusion_enabled": bool(config["model"].get("cache_home_summary_fusion", False)),
    }
...
def main() -> None:
    ...
    pilot_choices = {
        "baseline": choose_pilot(records, "baseline"),
        "fusion": choose_pilot(records, "fusion"),
    }
    ...
    for regime in ("c1", "c2"):
        baseline = [r for r in main_records if r["regime"] == regime and r["condition"] == "baseline"]
        fusion = [r for r in main_records if r["regime"] == regime and r["condition"] == "fusion"]
        main_summary[regime] = {
            "baseline": group_summary(baseline),
            "fusion": group_summary(fusion),
        }
        dense_gaps[regime] = (
            main_summary[regime]["fusion"].get("dense_mean", {}).get("mean", 0.0)
            - main_summary[regime]["baseline"].get("dense_mean", {}).get("mean", 0.0)
        )

    strongest_regime = max(dense_gaps, key=lambda key: dense_gaps[key]) if dense_gaps else "c2"
    rerun_summary = {
        "regime": strongest_regime,
        "baseline": group_summary([r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "baseline"]),
        "fusion": group_summary([r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "fusion"]),
    }

    probe_audit = {}
    for condition in ("baseline", "fusion"):
        candidates = [
            r for r in main_records if r["regime"] == "c2" and r["condition"] == condition
        ]
        if not candidates:
            probe_audit[condition] = {}
            continue
        best_run = max(candidates, key=lambda record: record["dense_mean"])
        probe_path = RUNS / best_run["run"] / "probe_best.pt"
        probe_audit[condition] = {
    ...
    positive = (
        ...
        and rerun_summary["fusion"].get("dense_mean", {}).get("mean", 0.0)
        >= rerun_summary["baseline"].get("dense_mean", {}).get("mean", 0.0)
    )

    summary = {
        "experiment": "v67_cache_summary_fusion_rescue",
        ...
        "positive": positive,
        "next_move": "keep exploring collision readout rescue" if positive else "do not promote this rescue as the main path",
        "runs": sorted(records, key=lambda r: (r["schedule"], r["regime"], r["condition"], r["seed"], r["tag"])),
    }
    ...
    lines = [
        ...
        "## Pilot Choices",
        "",
        f"- Baseline LR multiplier: `{pilot_choices['baseline'].get('lr_multiplier', 0.0)}`",
        f"- Fusion LR multiplier: `{pilot_choices['fusion'].get('lr_multiplier', 0.0)}`",
        "",
        "## Main Results",
        "",
    ]
    for regime in ("c1", "c2"):
        base = main_summary[regime]["baseline"].get("dense_mean", {}).get("mean", 0.0)
        fusion = main_summary[regime]["fusion"].get("dense_mean", {}).get("mean", 0.0)
        gap = dense_gaps.get(regime, 0.0)
        lines.append(f"- `{regime}` baseline dense `{base:.4f}`, fusion dense `{fusion:.4f}`, gap `{gap:+.4f}`")
    lines.extend(
        [
            "",
            "## Fresh Rerun",
            ...
            f"- Baseline dense: `{rerun_summary['baseline'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
            f"- Fusion dense: `{rerun_summary['fusion'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
            "",
            "## Probe Audit on C2 Best Checkpoints",
            "",
            f"- Baseline sink/cache/home probe test acc: `{probe_audit.get('baseline', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('baseline', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('baseline', {}).get('home_hidden_state', 0.0):.3f}`",
            f"- Fusion sink/cache/home probe test acc: `{probe_audit.get('fusion', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('fusion', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('fusion', {}).get('home_hidden_state', 0.0):.3f}`",
            "",
            "## Conclusion",
            "",
            f"- Positive: `{positive}`",

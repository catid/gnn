...
SUMMARY_PATH = REPORTS / "summary_metrics_v37.json"
REPORT_PATH = REPORTS / "final_report_v37_sparse_fraction_neighborhood.md"
...
SELECTORS = {
    ...
    "visit_taskgrad_half_agree_mutate_z00_m075_f0375": {"label": "VT-0.5 CAG-z0.0-m0.75-f0.375"},
}
...
PHASES = {
    "core_l": {
        ...
        "title": "Core-L Sparse Fraction Screen",
    },
    "t1_l": {
        "regime": "t1",
        "schedule": "l",
        ...
        "writers": [4, 8, 12, 14],
        "selectors": list(SELECTORS),
        "title": "T1-L Sparse Fraction Screen",
    },
    "core_xl": {
        "regime": "core",
        "schedule": "xl",
...
def phase_records(phase: str) -> list[dict[str, Any]]:
    ...
    for selector in spec["selectors"]:
        prefix = f"v37-{spec['regime']}-{selector}-32-{spec['schedule']}"
...
def run_table(records: list[dict[str, Any]]) -> str:
    headers = ["Phase", "Sel", "Seed", "Best", "Last", "Dense", "LastDense", "Comp"]
    rows: list[list[str]] = []
...
def plot_phase(summary: dict[str, dict[str, Any]], phase: str, path: Path) -> None:
    ...
    metrics = [("best_val", "Best"), ("last_val", "Last"), ("dense_mean", "Dense"), ("screen_composite", "Composite")]
...
def main() -> None:
    ...
    for phase, summary in phase_summaries.items():
        plot_phase(summary, phase, REPORTS / f"v37_{phase}_summary.png")
    ...
    lines.extend(
        [
            ...
            "## Outputs",
            "",
            f"- Summary JSON: [summary_metrics_v37.json]({SUMMARY_PATH})",
            f"- Report: [final_report_v37_sparse_fraction_neighborhood.md]({REPORT_PATH})",

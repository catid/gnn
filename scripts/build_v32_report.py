...
SUMMARY_PATH = REPORTS / "summary_metrics_v32.json"
REPORT_PATH = REPORTS / "final_report_v32_cag_margin_neighborhood.md"

SELECTORS = {
    "visit_taskgrad_half": {"label": "VT-0.5", "kind": "baseline"},
    "visit_taskgrad_half_agree_mutate_z00_m050": {"label": "VT-0.5 CAG-z0.0-m0.50", "kind": "mutation"},
    "visit_taskgrad_half_agree_mutate_z00_m075": {"label": "VT-0.5 CAG-z0.0-m0.75", "kind": "mutation"},
    "visit_taskgrad_half_agree_mutate_z00_m100": {"label": "VT-0.5 CAG-z0.0-m1.00", "kind": "mutation"},
}
...
def phase_records(phase: str) -> list[dict[str, Any]]:
    ...
    for selector in spec["selectors"]:
        prefix = f"v32-{spec['regime']}-{selector}-32-{spec['schedule']}"
...
def rank_mutation_arms(summary: dict[str, dict[str, Any]]) -> list[str]:
    ...
    return sorted(mutation_arms, key=lambda selector: summary[selector]["screen_composite"]["mean"], reverse=True)
...
def main() -> None:
    ...
    for phase, summary in phase_summaries.items():
        plot_phase(summary, phase, REPORTS / f"v32_{phase}_summary.png")
    ...
    if core_l_summary:
        ...
        if promoted_mutation:
            lines.append(f"Promoted mutation margin: `{SELECTORS[promoted_mutation]['label']}`")
            lines.append("")
    ...
    lines.extend(
        [
            ...
            "## Outputs",
            "",
            f"- Summary JSON: [summary_metrics_v32.json]({SUMMARY_PATH})",
            f"- Report: [final_report_v32_cag_margin_neighborhood.md]({REPORT_PATH})",

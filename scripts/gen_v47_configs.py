...
ARMS = {
    ...
    "stageadaptive_late_quarter": {
        ...
        "growth": {
            ...
            "adaptive_utility_grad_weight": 0.25,
...
def write_script(path: Path, config_name: str, run_name: str) -> None:
    ...
    path.write_text(
        f"#!/usr/bin/env bash\nCONFIG=configs/{config_name} DEFAULT_SEED=120234 DEFAULT_RUN_NAME={run_name} exec \"$(dirname \"$0\")/_train_wrapper_single_gpu.sh\"\n",
        encoding="utf-8",
    )
    path.chmod(0o755)
...
def build_config(arm: str, regime: str) -> dict:
    return {
        ...
        "runtime": {
            "run_name": f"v47-{regime}-{arm}-32-xl",
...
def main() -> None:
    ...
    for arm in ARMS:
        for regime in REGIMES:
            config_name = f"v47_{regime}_{arm}_32_xl.yaml"
            run_name = f"v47-{regime}-{arm}-32-xl"
            dump_yaml(build_config(arm, regime), CONFIGS / config_name)
            write_script(SCRIPTS / f"train_v47_{regime}_{arm}_32_xl.sh", config_name, run_name)

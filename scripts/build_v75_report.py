...
SUMMARY_PATH = REPORTS / "summary_metrics_v75.json"
REPORT_PATH = REPORTS / "final_report_v75_ambiguity_aware_output_gate.md"
PACK_DEFS_PATH = REPORTS / "v75_pack_definitions.json"
BASE_LR = 2.0e-4
EXPECTED_TRAIN_STEPS = {"p": 300, "m": 900, "l": 1350}
RUN_RE = re.compile(
    r"v75-collision-(?P<regime>c[12])-(?P<condition>mean|ambig)-(?P<pair>[a-z0-9_]+)-32-(?P<schedule>p|m|l)(?:-(?P<tag>(?!s\d+$)[^-]+))?(?:-s(?P<seed>\d+))?$"
)
...
def latest_runs() -> list[tuple[dict[str, str], Path]]:
    ...
    for candidate in sorted(RUNS.glob("*-v75-*")):
...
def main() -> None:
    ...
    positive = (
        ...
        and rerun_summary["ambig"].get("dense_mean", {}).get("mean", 0.0)
        >= rerun_summary["mean"].get("dense_mean", {}).get("mean", 0.0)
    )

    summary = {
        "experiment": "v75_ambiguity_aware_output_gate",
    ...
    report_lines = [
        ...
        "## Main Results",
        "",
        f"- `c1` mean dense `{main_summary['c1']['mean'].get('dense_mean', {}).get('mean', 0.0):.4f}`, ambig dense `{main_summary['c1']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c1', 0.0):+.4f}`",
        f"- `c2` mean dense `{main_summary['c2']['mean'].get('dense_mean', {}).get('mean', 0.0):.4f}`, ambig dense `{main_summary['c2']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c2', 0.0):+.4f}`",
        "",
        "## Fresh Rerun",
        "",

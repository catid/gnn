...
SUMMARY_PATH = REPORTS / "summary_metrics_v41.json"
REPORT_PATH = REPORTS / "final_report_v41_late_stage_adaptive_transfer_recheck.md"
...
PHASES = {
    "t1_xl": {
        ...
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "title": "T1-XL Fresh Transfer Recheck",
    },
    "t1r_xl": {
        "regime": "t1r",
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "title": "T1-XL Rerun Recheck",
    },
    "t2a_xl": {
        "regime": "t2a",
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "title": "T2a-XL Ingress Stress Recheck",
    },
}
...
def phase_records(phase: str) -> list[dict[str, Any]]:
    ...
    for selector in SELECTORS:
        prefix = f"v41-{spec['regime']}-{selector}-32-xl"
...
def main() -> None:
    ...
    for phase, summary in phase_summaries.items():
        plot_phase(summary, phase, REPORTS / f"v41_{phase}_summary.png")
    ...
    REPORT_PATH.write_text(
        "\n".join(
            [
                "# APSGNN v41: Late-Stage Adaptive Transfer Recheck",
                "",
                "## What Changed",
                "",
                "v41 rechecks the only live regime-dependent selector alternative after the sparse-package line was retired.",
                "It compares mutation-free `VT-0.5` against the exact old `StageLate-0.5` arm on fresh transfer, rerun transfer, and ingress stress.",
                "",
                f"- Summary JSON: [summary_metrics_v41.json]({SUMMARY_PATH})",
                f"- Report: [final_report_v41_late_stage_adaptive_transfer_recheck.md]({REPORT_PATH})",
                "",
            ]
        ),
        encoding="utf-8",

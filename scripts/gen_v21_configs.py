...
SCHEDULES = {
    "s": {
        "train_steps": 1530,
        "stage_steps": [100, 100, 120, 120, 150, 170, 770],
        "eval_interval": 153,
        "save_interval": 153,
        "anneal_steps": 500,
    },
    "m": {
        "train_steps": 2550,
        "stage_steps": [120, 120, 150, 150, 180, 220, 1610],
        "eval_interval": 170,
        "save_interval": 170,
        "anneal_steps": 700,
    },
...
NAME_MAP = {
    "visitonly": "visitonly",
    "visit_taskgrad": "visit_taskgrad",
    "visit_querygrad": "visit_querygrad",
    "full_querygrad": "full_querygrad",
    "querygradonly": "querygradonly",
    "visit_taskgrad_half": "visit_taskgrad_half",
    "visit_querygrad_half": "visit_querygrad_half",
}
...
def build_config(selector: str, regime: str, schedule: str) -> dict:
    ...
    return {
        ...
        "runtime": {
            "run_name": f"v21-{regime}-{NAME_MAP[selector]}-32-{schedule}",
        },
        "growth": {
    ...
    for selector in SELECTORS:
        ...
        for regime, schedules in regime_schedules.items():
            for schedule in schedules:
                config_name = f"v21_{regime}_{NAME_MAP[selector]}_32_{schedule}.yaml"
                dump_yaml(build_config(selector, regime, schedule), CONFIGS / config_name)
                write_script(
                    SCRIPTS / f"train_v21_{regime}_{NAME_MAP[selector]}_32_{schedule}.sh",
                    config_name,
                    f"v21-{regime}-{NAME_MAP[selector]}-32-{schedule}",
                    smoke=False,
                )
                if schedule == "s":
                    write_script(
                        SCRIPTS / f"smoke_v21_{regime}_{NAME_MAP[selector]}_32_{schedule}.sh",
                        config_name,
                        f"v21-{regime}-{NAME_MAP[selector]}-32-{schedule}",
                        smoke=True,
                    )

        config_name = f"v21_t2a_{NAME_MAP[selector]}_32_l.yaml"
        dump_yaml(build_config(selector, "t2a", "l"), CONFIGS / config_name)
        write_script(
            SCRIPTS / f"train_v21_t2a_{NAME_MAP[selector]}_32_l.sh",
            config_name,
            f"v21-t2a-{NAME_MAP[selector]}-32-l",
            smoke=False,
        )

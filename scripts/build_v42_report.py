...
SUMMARY_PATH = REPORTS / "summary_metrics_v42.json"
REPORT_PATH = REPORTS / "final_report_v42_matched_regime_rule_confirmation.md"
...
PHASES = {
    "core_xl": {
        ...
        "train_steps": 4590,
        "writers": [2, 6, 10],
        "title": "Core-XL Matched Confirmation",
    },
    "t1_xl": {
        "regime": "t1",
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "title": "T1-XL Matched Confirmation",
    },
    "t1r_xl": {
        "regime": "t1r",
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "title": "T1-XL Rerun Confirmation",
    },
    "t2a_xl": {
        "regime": "t2a",
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "title": "T2a-XL Stress Confirmation",
    },
}
...
def phase_records(phase: str) -> list[dict[str, Any]]:
    ...
    for selector in SELECTORS:
        prefix = f"v42-{spec['regime']}-{selector}-32-xl"
...
def main() -> None:
    ...
    for phase, summary in phase_summaries.items():
        plot_phase(summary, phase, REPORTS / f"v42_{phase}_summary.png")
    ...
    if final_recommendation == "visit_taskgrad_half":
        recommendation = "Matched v42 evidence favors a single global default: VT-0.5."
    else:
        recommendation = "Matched v42 evidence favors a regime-dependent rule: VT-0.5 on home, StageLate-0.5 on transfer/stress."

    REPORT_PATH.write_text(
        "\n".join(
            [
                "# APSGNN v42: Matched Regime Rule Confirmation",
                "",
                "## What Changed",
                "",
                "v42 is the direct follow-up to the split v41 result.",
                "It compares VT-0.5 and StageLate-0.5 on the same fresh seed set across home, transfer, rerun transfer, and ingress stress so the regime-rule claim stands or falls on one matched matrix.",
                "",
                f"## Recommendation\n\n{recommendation}\n",
                f"- Summary JSON: [summary_metrics_v42.json]({SUMMARY_PATH})",
                f"- Report: [final_report_v42_matched_regime_rule_confirmation.md]({REPORT_PATH})",
                "",
            ]
        ),
        encoding="utf-8",

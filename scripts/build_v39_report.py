...
SUMMARY_PATH = REPORTS / "summary_metrics_v39.json"
REPORT_PATH = REPORTS / "final_report_v39_sparse_transfer_specialist_validation.md"
...
PHASES = {
    "t1_xl": {
        ...
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "title": "T1-XL Fresh Transfer Confirmation",
    },
    "t1r_xl": {
        "regime": "t1r",
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "title": "T1-XL Rerun Transfer Validation",
    },
    "t2b_xl": {
        "regime": "t2b",
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "title": "T2b-XL Tight-TTL Stress",
    },
}
...
def phase_records(phase: str) -> list[dict[str, Any]]:
    ...
    for selector in SELECTORS:
        prefix = f"v39-{spec['regime']}-{selector}-32-xl"
...
def main() -> None:
    ...
    for phase, summary in phase_summaries.items():
        plot_phase(summary, phase, REPORTS / f"v39_{phase}_summary.png")
    ...
    if sparse > baseline:
        ...
        recommendation_lines = [
            "Mutation-free `VT-0.5` still wins the transfer-focused v39 score.",
            "That means the sparse package should be retired rather than kept as a regime-dependent specialist.",
        ]
    lines = [
    ...
    lines.extend(
        [
            ...
            *recommendation_lines,
            ...
            "## Outputs",
            "",
            f"- Summary JSON: [summary_metrics_v39.json]({SUMMARY_PATH})",
            f"- Report: [final_report_v39_sparse_transfer_specialist_validation.md]({REPORT_PATH})",

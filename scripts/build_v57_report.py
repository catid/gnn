...
SUMMARY_PATH = REPORTS / "summary_metrics_v57.json"
REPORT_PATH = REPORTS / "final_report_v57_final_rule_recheck.md"
...
RULES = {
    ...
    "regime_keyed_rule": {
        "label": "VT-0.5 on Core/T2a, StageLate-0.5 on T1/T1r/T2b/T2c",
        "selection": {
            "home": "visit_taskgrad_half",
            "transfer": "stageadaptive_late_half",
            "ingress": "visit_taskgrad_half",
            "non_ingress_stress": "stageadaptive_late_half",
        },
    },
}
...
PHASES = {
    ...
    "pooled_core_xl": {
        "prefixes": ["v51-core", "v57-core"],
        "train_steps": 4590,
        "writers": [2, 6, 10],
        "family": "home",
    },
    "pooled_t1_xl": {
        "prefixes": ["v51-t1", "v57-t1"],
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "family": "transfer",
    },
    "pooled_t1r_xl": {
        "prefixes": ["v51-t1r", "v57-t1r"],
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "family": "transfer",
    },
    "pooled_t2a_xl": {
        "prefixes": ["v51-t2a", "v57-t2a"],
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "family": "ingress",
    },
    "pooled_t2b_xl": {
        "prefixes": ["v51-t2b", "v57-t2b"],
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "family": "non_ingress_stress",
    },
    "pooled_t2c_xl": {
        "prefixes": ["v51-t2c", "v57-t2c"],
        "train_steps": 4590,
        "writers": [6, 10, 14, 16],
        "family": "non_ingress_stress",
    },
    ...
    REPORT_PATH.write_text(
        "\n".join(
            [
                ...
                f"Pooled v51+v57 rule winner: `{pooled_rule_recommendation}` ({RULES[pooled_rule_recommendation]['label']}).",
                "",
                f"- Summary JSON: [summary_metrics_v57.json]({SUMMARY_PATH})",
                f"- Report: [final_report_v57_final_rule_recheck.md]({REPORT_PATH})",
                "",
            ]
        ),
        encoding="utf-8",

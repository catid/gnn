...
SUMMARY_PATH = REPORTS / "summary_metrics_v73.json"
REPORT_PATH = REPORTS / "final_report_v73_dispersion_augmented_output_readout.md"
PACK_DEFS_PATH = REPORTS / "v73_pack_definitions.json"
BASE_LR = 2.0e-4
EXPECTED_TRAIN_STEPS = {"p": 300, "m": 900, "l": 1350}
RUN_RE = re.compile(
    r"v73-collision-(?P<regime>c[12])-(?P<condition>mean|disp)-(?P<pair>[a-z0-9_]+)-32-(?P<schedule>p|m|l)(?:-(?P<tag>(?!s\d+$)[^-]+))?(?:-s(?P<seed>\d+))?$"
)
...
def latest_runs() -> list[tuple[dict[str, str], Path]]:
    ...
    for candidate in sorted(RUNS.glob("*-v73-*")):
...
def summarize_run(run_dir: Path, meta: dict[str, str]) -> dict[str, Any]:
    ...
    record = {
        ...
        "dispersion_scale": float(config["model"].get("cache_output_dispersion_scale", 0.0)),
    }
...
def main() -> None:
    ...
    pilot_choices = {
        "mean": choose_pilot(records, "mean"),
        "disp": choose_pilot(records, "disp"),
    }
    ...
    for regime in ("c1", "c2"):
        mean_records = [r for r in main_records if r["regime"] == regime and r["condition"] == "mean"]
        disp_records = [r for r in main_records if r["regime"] == regime and r["condition"] == "disp"]
        main_summary[regime] = {
            "mean": group_summary(mean_records),
            "disp": group_summary(disp_records),
        }
        dense_gaps[regime] = (
            main_summary[regime]["disp"].get("dense_mean", {}).get("mean", 0.0)
            - main_summary[regime]["mean"].get("dense_mean", {}).get("mean", 0.0)
        )

    strongest_regime = max(dense_gaps, key=lambda key: dense_gaps[key]) if dense_gaps else "c2"
    rerun_summary = {
        "regime": strongest_regime,
        "mean": group_summary([r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "mean"]),
        "disp": group_summary([r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "disp"]),
    }

    probe_audit: dict[str, dict[str, Any]] = {}
    for condition in ("mean", "disp"):
        ...
        probe_audit[condition] = {
            ...
            "home_cache_dispersion_state": probe_accuracy(probe_path, "home_cache_dispersion_state"),
    ...
    positive = (
        ...
        and rerun_summary["disp"].get("dense_mean", {}).get("mean", 0.0)
        >= rerun_summary["mean"].get("dense_mean", {}).get("mean", 0.0)
    )

    summary = {
        "experiment": "v73_dispersion_augmented_output_readout",
    ...
    report_lines = [
        ...
        "## Pilot Choices",
        "",
        f"- Mean-readout LR multiplier: `{pilot_choices['mean'].get('lr_multiplier', 0.0):.1f}`",
        f"- Dispersion-augmented LR multiplier: `{pilot_choices['disp'].get('lr_multiplier', 0.0):.1f}`",
        "",
        "## Main Results",
        "",
        f"- `c1` mean dense `{main_summary['c1']['mean'].get('dense_mean', {}).get('mean', 0.0):.4f}`, disp dense `{main_summary['c1']['disp'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c1', 0.0):+.4f}`",
        f"- `c2` mean dense `{main_summary['c2']['mean'].get('dense_mean', {}).get('mean', 0.0):.4f}`, disp dense `{main_summary['c2']['disp'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c2', 0.0):+.4f}`",
        "",
        "## Fresh Rerun",
        "",
        f"- Regime: `{strongest_regime}`",
        f"- Mean dense: `{rerun_summary['mean'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        f"- Dispersion-augmented dense: `{rerun_summary['disp'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        "",
        "## Probe Audit on C2 Best Checkpoints",
        "",
        f"- Mean sink/cache/cache-disp/home probe test acc: `{probe_audit.get('mean', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('mean', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('mean', {}).get('home_cache_dispersion_state', 0.0):.3f}` / `{probe_audit.get('mean', {}).get('home_hidden_state', 0.0):.3f}`",
        f"- Dispersion-augmented sink/cache/cache-disp/home probe test acc: `{probe_audit.get('disp', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('disp', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('disp', {}).get('home_cache_dispersion_state', 0.0):.3f}` / `{probe_audit.get('disp', {}).get('home_hidden_state', 0.0):.3f}`",
        "",
        "## Conclusion",
        "",

...
SUMMARY_PATH = REPORTS / "summary_metrics_v79.json"
REPORT_PATH = REPORTS / "final_report_v79_ambiguity_conditioned_base_suppression.md"
PACK_DEFS_PATH = REPORTS / "v79_pack_definitions.json"
BASE_LR = 2.0e-4
EXPECTED_TRAIN_STEPS = {"p": 300, "m": 900, "l": 1350}
RUN_RE = re.compile(
    r"v79-collision-(?P<regime>c[12])-(?P<condition>ambig|ambigbase)-(?P<pair>[a-z0-9_]+)-32-(?P<schedule>p|m|l)(?:-(?P<tag>(?!s\d+$)[^-]+))?(?:-s(?P<seed>\d+))?$"
)
...
def latest_runs() -> list[tuple[dict[str, str], Path]]:
    ...
    for candidate in sorted(RUNS.glob("*-v79-*")):
...
def main() -> None:
    ...
    pilot_choices = {
        ...
        "ambigbase": choose_pilot(records, "ambigbase"),
    }
    ...
    for regime in ("c1", "c2"):
        ...
        ambigbase_records = [r for r in main_records if r["regime"] == regime and r["condition"] == "ambigbase"]
        main_summary[regime] = {
            "ambig": group_summary(ambig_records),
            "ambigbase": group_summary(ambigbase_records),
        }
        dense_gaps[regime] = (
            main_summary[regime]["ambigbase"].get("dense_mean", {}).get("mean", 0.0)
    ...
    rerun_summary = {
        ...
        "ambigbase": group_summary(
            [r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "ambigbase"]
        ),
    }

    probe_audit: dict[str, dict[str, Any]] = {}
    for condition in ("ambig", "ambigbase"):
    ...
    positive = (
        ...
        and rerun_summary["ambigbase"].get("dense_mean", {}).get("mean", 0.0)
        >= rerun_summary["ambig"].get("dense_mean", {}).get("mean", 0.0)
    )

    summary = {
        "experiment": "v79_ambiguity_conditioned_base_suppression",
    ...
    report_lines = [
        ...
        f"- Ambiguity-base-suppressed LR multiplier: `{pilot_choices['ambigbase'].get('lr_multiplier', 0.0):.1f}`",
        "",
        "## Main Results",
        "",
        f"- `c1` ambig dense `{main_summary['c1']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, ambigbase dense `{main_summary['c1']['ambigbase'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c1', 0.0):+.4f}`",
        f"- `c2` ambig dense `{main_summary['c2']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, ambigbase dense `{main_summary['c2']['ambigbase'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c2', 0.0):+.4f}`",
        "",
        "## Fresh Rerun",
        "",
        f"- Regime: `{strongest_regime}`",
        f"- Ambiguity-aware dense: `{rerun_summary['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        f"- Ambiguity-base-suppressed dense: `{rerun_summary['ambigbase'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        "",
        ...
        f"- Ambiguity-base-suppressed sink/cache/cache-max/home probe test acc: `{probe_audit.get('ambigbase', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('ambigbase', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('ambigbase', {}).get('home_cache_max_state', 0.0):.3f}` / `{probe_audit.get('ambigbase', {}).get('home_hidden_state', 0.0):.3f}`",
        "",
        "## Conclusion",
        "",

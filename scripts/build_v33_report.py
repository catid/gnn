...
SUMMARY_PATH = REPORTS / "summary_metrics_v33.json"
REPORT_PATH = REPORTS / "final_report_v33_default_package_confirmation.md"

SELECTORS = {
    "visit_taskgrad_half": {"label": "VT-0.5"},
    "visit_taskgrad_half_agree_mutate_z00_m075": {"label": "VT-0.5 CAG-z0.0-m0.75"},
}
...
def phase_records(phase: str) -> list[dict[str, Any]]:
    ...
    for selector in SELECTORS:
        prefix = f"v33-{regime}-{selector}-32-xl"
...
def main() -> None:
    ...
    for phase, summary in phase_summaries.items():
        plot_phase(summary, phase, REPORTS / f"v33_{phase}_summary.png")
    ...
    lines.extend(
        [
            ...
            "## Outputs",
            "",
            f"- Summary JSON: [summary_metrics_v33.json]({SUMMARY_PATH})",
            f"- Report: [final_report_v33_default_package_confirmation.md]({REPORT_PATH})",

...
SUMMARY_PATH = REPORTS / "summary_metrics_v45.json"
REPORT_PATH = REPORTS / "final_report_v45_regime_dependent_package_rule_confirmation.md"

SELECTORS = {
    "visit_taskgrad_half": {"label": "VT-0.5"},
    "stageadaptive_late_half_agree_mutate_z00_m075_f0125": {
        "label": "StageLate-0.5 CAG-z0.0-m0.75-f0.125"
    },
}

PHASES = {
    "core_xl": {"regime": "core", "train_steps": 4590, "writers": [2, 6, 10]},
...
def phase_records(phase: str) -> list[dict[str, Any]]:
    ...
    for selector in SELECTORS:
        prefix = f"v45-{spec['regime']}-{selector}-32-xl"
...
def main() -> None:
    ...
    if final_recommendation == "visit_taskgrad_half":
        recommendation = "v45 favors a single global default: VT-0.5."
    else:
        recommendation = "v45 favors the regime-dependent package rule: VT-0.5 on home, StageLate-0.5 CAG-z0.0-m0.75-f0.125 on transfer/rerun/stress."

    REPORT_PATH.write_text(
        "\n".join(
            [
                "# APSGNN v45: Regime-Dependent Package Rule Confirmation",
                "",
                "## What Changed",
                "",
                "v45 directly compares the home default VT-0.5 against the specialist package StageLate-0.5 CAG-z0.0-m0.75-f0.125 on a matched Core/T1/T1r/T2a XL matrix.",
                "",
                "## Recommendation",
                "",
                recommendation,
                "",
                f"- Summary JSON: [summary_metrics_v45.json]({SUMMARY_PATH})",
                f"- Report: [final_report_v45_regime_dependent_package_rule_confirmation.md]({REPORT_PATH})",
                "",
            ]
        ),
        encoding="utf-8",

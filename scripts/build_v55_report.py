...
SUMMARY_PATH = REPORTS / "summary_metrics_v55.json"
REPORT_PATH = REPORTS / "final_report_v55_stagelate_t2b_package_tiebreak.md"
...
PHASES = {
    "fresh_t2b_xl": {"prefixes": ["v55-t2b"], "train_steps": 4590, "writers": [4, 8, 12, 14]},
    "pooled_t2b_xl": {"prefixes": ["v54-t2b", "v55-t2b"], "train_steps": 4590, "writers": [4, 8, 12, 14]},
}
...
def main() -> None:
    ...
    REPORT_PATH.write_text(
        "\n".join(
            [
                "# APSGNN v55: StageLate T2b Package Tie-Break",
                "",
                "## What Changed",
                "",
                "v55 isolates the only remaining specialist-package holdout: T2b. It reruns StageLate-0.5 versus its lightweight sparse CAG package on fresh same-seed T2b-XL runs, then pools those runs with the earlier v54 T2b slice.",
                "",
                "## Recommendation",
                "",
                f"Current overall winner on the fresh+pooled T2b package boundary: {SELECTORS[final_recommendation]['label']}.",
                "",
                f"- Summary JSON: [summary_metrics_v55.json]({SUMMARY_PATH})",
                f"- Report: [final_report_v55_stagelate_t2b_package_tiebreak.md]({REPORT_PATH})",
                "",
            ]
        ),
        encoding="utf-8",

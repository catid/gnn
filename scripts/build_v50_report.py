...
SUMMARY_PATH = REPORTS / "summary_metrics_v50.json"
REPORT_PATH = REPORTS / "final_report_v50_stress_axis_map.md"
...
def phase_records(phase: str) -> list[dict[str, Any]]:
    ...
    for selector in SELECTORS:
        prefix = f"v50-{spec['regime']}-{selector}-32-xl"
...
def main() -> None:
    ...
    REPORT_PATH.write_text(
        "\n".join(
            [
                "# APSGNN v50: Stress-Axis Map",
                "",
                "## What Changed",
                "",
                "v50 maps the remaining stress question directly by comparing VT-0.5 and StageLate-0.5 on T2b and T2c, after v46-v49 established that T1/T1r favor StageLate-0.5 while T2a favors VT-0.5.",
                "",
                f"- Summary JSON: [summary_metrics_v50.json]({SUMMARY_PATH})",
                f"- Report: [final_report_v50_stress_axis_map.md]({REPORT_PATH})",
                "",
            ]
        ),
        encoding="utf-8",

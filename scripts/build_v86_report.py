...
SUMMARY_PATH = REPORTS / "summary_metrics_v86.json"
REPORT_PATH = REPORTS / "final_report_v86_collision_specialized_output_head.md"
PACK_DEFS_PATH = REPORTS / "v86_pack_definitions.json"
BASE_LR = 2.0e-4
EXPECTED_TRAIN_STEPS = {"p": 300, "m": 900, "l": 1350}
RUN_RE = re.compile(
    r"v86-collision-(?P<regime>c[12])-(?P<condition>ambig|splithead)-(?P<pair>[a-z0-9_]+)-32-(?P<schedule>p|m|l)(?:-(?P<tag>(?!s\d+$)[^-]+))?(?:-s(?P<seed>\d+))?$"
)
...
def latest_runs() -> list[tuple[dict[str, str], Path]]:
    ...
    for candidate in sorted(RUNS.glob("*-v86-*")):
...
def main() -> None:
    ...
    pilot_choices = {
        ...
        "splithead": choose_pilot(records, "splithead"),
    }
    ...
    for regime in ("c1", "c2"):
        ...
        splithead_records = [r for r in main_records if r["regime"] == regime and r["condition"] == "splithead"]
        main_summary[regime] = {
            "ambig": group_summary(ambig_records),
            "splithead": group_summary(splithead_records),
        }
        dense_gaps[regime] = (
            main_summary[regime]["splithead"].get("dense_mean", {}).get("mean", 0.0)
    ...
    rerun_summary = {
        ...
        "splithead": group_summary(
            [r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "splithead"]
        ),
    }

    probe_audit: dict[str, dict[str, Any]] = {}
    for condition in ("ambig", "splithead"):
    ...
    positive = (
        ...
        and rerun_summary["splithead"].get("dense_mean", {}).get("mean", 0.0)
        >= rerun_summary["ambig"].get("dense_mean", {}).get("mean", 0.0)
    )

    summary = {
        "experiment": "v86_collision_specialized_output_head",
    ...
    report_lines = [
        ...
        f"- Collision-specialized-head LR multiplier: `{pilot_choices['splithead'].get('lr_multiplier', 0.0):.1f}`",
        "",
        "## Main Results",
        "",
        f"- `c1` ambig dense `{main_summary['c1']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, splithead dense `{main_summary['c1']['splithead'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c1', 0.0):+.4f}`",
        f"- `c2` ambig dense `{main_summary['c2']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, splithead dense `{main_summary['c2']['splithead'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c2', 0.0):+.4f}`",
        "",
        "## Fresh Rerun",
        "",
        f"- Regime: `{strongest_regime}`",
        f"- Ambiguity-aware dense: `{rerun_summary['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        f"- Collision-specialized-head dense: `{rerun_summary['splithead'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        "",
        ...
        f"- Collision-specialized-head sink/cache/cache-max/home probe test acc: `{probe_audit.get('splithead', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('splithead', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('splithead', {}).get('home_cache_max_state', 0.0):.3f}` / `{probe_audit.get('splithead', {}).get('home_hidden_state', 0.0):.3f}`",
        "",
        "## Conclusion",
        "",

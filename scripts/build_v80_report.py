...
SUMMARY_PATH = REPORTS / "summary_metrics_v80.json"
REPORT_PATH = REPORTS / "final_report_v80_ambiguity_conditioned_classslice_suppression.md"
PACK_DEFS_PATH = REPORTS / "v80_pack_definitions.json"
BASE_LR = 2.0e-4
EXPECTED_TRAIN_STEPS = {"p": 300, "m": 900, "l": 1350}
RUN_RE = re.compile(
    r"v80-collision-(?P<regime>c[12])-(?P<condition>ambig|ambigclass)-(?P<pair>[a-z0-9_]+)-32-(?P<schedule>p|m|l)(?:-(?P<tag>(?!s\d+$)[^-]+))?(?:-s(?P<seed>\d+))?$"
)
...
def latest_runs() -> list[tuple[dict[str, str], Path]]:
    ...
    for candidate in sorted(RUNS.glob("*-v80-*")):
...
def main() -> None:
    ...
    pilot_choices = {
        ...
        "ambigclass": choose_pilot(records, "ambigclass"),
    }
    ...
    for regime in ("c1", "c2"):
        ...
        ambigclass_records = [r for r in main_records if r["regime"] == regime and r["condition"] == "ambigclass"]
        main_summary[regime] = {
            "ambig": group_summary(ambig_records),
            "ambigclass": group_summary(ambigclass_records),
        }
        dense_gaps[regime] = (
            main_summary[regime]["ambigclass"].get("dense_mean", {}).get("mean", 0.0)
    ...
    rerun_summary = {
        ...
        "ambigclass": group_summary(
            [r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "ambigclass"]
        ),
    }

    probe_audit: dict[str, dict[str, Any]] = {}
    for condition in ("ambig", "ambigclass"):
    ...
    positive = (
        ...
        and rerun_summary["ambigclass"].get("dense_mean", {}).get("mean", 0.0)
        >= rerun_summary["ambig"].get("dense_mean", {}).get("mean", 0.0)
    )

    summary = {
        "experiment": "v80_ambiguity_conditioned_classslice_suppression",
    ...
    report_lines = [
        ...
        f"- Ambiguity-class-suppressed LR multiplier: `{pilot_choices['ambigclass'].get('lr_multiplier', 0.0):.1f}`",
        "",
        "## Main Results",
        "",
        f"- `c1` ambig dense `{main_summary['c1']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, ambigclass dense `{main_summary['c1']['ambigclass'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c1', 0.0):+.4f}`",
        f"- `c2` ambig dense `{main_summary['c2']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, ambigclass dense `{main_summary['c2']['ambigclass'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c2', 0.0):+.4f}`",
        "",
        "## Fresh Rerun",
        "",
        f"- Regime: `{strongest_regime}`",
        f"- Ambiguity-aware dense: `{rerun_summary['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        f"- Ambiguity-class-suppressed dense: `{rerun_summary['ambigclass'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        "",
        ...
        f"- Ambiguity-class-suppressed sink/cache/cache-max/home probe test acc: `{probe_audit.get('ambigclass', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('ambigclass', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('ambigclass', {}).get('home_cache_max_state', 0.0):.3f}` / `{probe_audit.get('ambigclass', {}).get('home_hidden_state', 0.0):.3f}`",
        "",
        "## Conclusion",
        "",

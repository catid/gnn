...
SUMMARY_PATH = REPORTS / "summary_metrics_v44.json"
REPORT_PATH = REPORTS / "final_report_v44_stageadaptive_specialist_package_check.md"
...
SELECTORS = {
    ...
    "stageadaptive_late_half_agree_mutate_z00_m075_f025": {"label": "StageLate-0.5 CAG-z0.0-m0.75-f0.25"},
}
...
def phase_records(phase: str) -> list[dict[str, Any]]:
    ...
    for selector in SELECTORS:
        prefix = f"v44-{spec['regime']}-{selector}-32-xl"
...
def main() -> None:
    ...
    REPORT_PATH.write_text(
        "\n".join(
            [
                "# APSGNN v44: StageAdaptive Specialist Package Check",
                "",
                "## What Changed",
                "",
                "v44 checks whether the restored StageLate-0.5 transfer/stress specialist should remain mutation-free or adopt a lightweight sparse CAG package.",
                "",
                f"- Summary JSON: [summary_metrics_v44.json]({SUMMARY_PATH})",
                f"- Report: [final_report_v44_stageadaptive_specialist_package_check.md]({REPORT_PATH})",
                "",
            ]
        ),
        encoding="utf-8",

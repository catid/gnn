...
SUMMARY_PATH = REPORTS / "summary_metrics_v81.json"
REPORT_PATH = REPORTS / "final_report_v81_entropy_aware_output_gate.md"
PACK_DEFS_PATH = REPORTS / "v81_pack_definitions.json"
BASE_LR = 2.0e-4
...
def latest_runs() -> list[tuple[dict[str, str], Path]]:
    ...
    for candidate in sorted(RUNS.glob("*-v81-*")):
...
def main() -> None:
    ...
    pilot_choices = {
        ...
        "ambigent": choose_pilot(records, "ambigent"),
    }
    ...
    for regime in ("c1", "c2"):
        ...
        ambigent_records = [r for r in main_records if r["regime"] == regime and r["condition"] == "ambigent"]
        main_summary[regime] = {
            "ambig": group_summary(ambig_records),
            "ambigent": group_summary(ambigent_records),
        }
        dense_gaps[regime] = (
            main_summary[regime]["ambigent"].get("dense_mean", {}).get("mean", 0.0)
    ...
    rerun_summary = {
        ...
        "ambigent": group_summary(
            [r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "ambigent"]
        ),
    }

    probe_audit: dict[str, dict[str, Any]] = {}
    for condition in ("ambig", "ambigent"):
    ...
    positive = (
        ...
        and rerun_summary["ambigent"].get("dense_mean", {}).get("mean", 0.0)
        >= rerun_summary["ambig"].get("dense_mean", {}).get("mean", 0.0)
    )

    summary = {
        "experiment": "v81_entropy_aware_output_gate",
    ...
    report_lines = [
        ...
        f"- Entropy-aware LR multiplier: `{pilot_choices['ambigent'].get('lr_multiplier', 0.0):.1f}`",
        "",
        "## Main Results",
        "",
        f"- `c1` ambig dense `{main_summary['c1']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, ambigent dense `{main_summary['c1']['ambigent'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c1', 0.0):+.4f}`",
        f"- `c2` ambig dense `{main_summary['c2']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, ambigent dense `{main_summary['c2']['ambigent'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c2', 0.0):+.4f}`",
        "",
        "## Fresh Rerun",
        "",
        f"- Regime: `{strongest_regime}`",
        f"- Ambiguity-aware dense: `{rerun_summary['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        f"- Entropy-aware dense: `{rerun_summary['ambigent'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        "",
        ...
        f"- Entropy-aware sink/cache/cache-max/home probe test acc: `{probe_audit.get('ambigent', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('ambigent', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('ambigent', {}).get('home_cache_max_state', 0.0):.3f}` / `{probe_audit.get('ambigent', {}).get('home_hidden_state', 0.0):.3f}`",
        "",
        "## Conclusion",
        "",

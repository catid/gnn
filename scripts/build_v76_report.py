...
SUMMARY_PATH = REPORTS / "summary_metrics_v76.json"
REPORT_PATH = REPORTS / "final_report_v76_ambiguity_conditioned_max_correction.md"
PACK_DEFS_PATH = REPORTS / "v76_pack_definitions.json"
BASE_LR = 2.0e-4
EXPECTED_TRAIN_STEPS = {"p": 300, "m": 900, "l": 1350}
RUN_RE = re.compile(
    r"v76-collision-(?P<regime>c[12])-(?P<condition>ambig|ambigmax)-(?P<pair>[a-z0-9_]+)-32-(?P<schedule>p|m|l)(?:-(?P<tag>(?!s\d+$)[^-]+))?(?:-s(?P<seed>\d+))?$"
)
...
def latest_runs() -> list[tuple[dict[str, str], Path]]:
    ...
    for candidate in sorted(RUNS.glob("*-v76-*")):
...
def main() -> None:
    ...
    pilot_choices = {
        ...
        "ambigmax": choose_pilot(records, "ambigmax"),
    }
    ...
    for regime in ("c1", "c2"):
        ...
        ambigmax_records = [r for r in main_records if r["regime"] == regime and r["condition"] == "ambigmax"]
        main_summary[regime] = {
            "ambig": group_summary(ambig_records),
            "ambigmax": group_summary(ambigmax_records),
        }
        dense_gaps[regime] = (
            main_summary[regime]["ambigmax"].get("dense_mean", {}).get("mean", 0.0)
    ...
    rerun_summary = {
        ...
        "ambigmax": group_summary(
            [r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "ambigmax"]
        ),
    }

    probe_audit: dict[str, dict[str, Any]] = {}
    for condition in ("ambig", "ambigmax"):
    ...
    positive = (
        ...
        and rerun_summary["ambigmax"].get("dense_mean", {}).get("mean", 0.0)
        >= rerun_summary["ambig"].get("dense_mean", {}).get("mean", 0.0)
    )

    summary = {
        "experiment": "v76_ambiguity_conditioned_max_correction",
    ...
    report_lines = [
        ...
        f"- Ambiguity+max LR multiplier: `{pilot_choices['ambigmax'].get('lr_multiplier', 0.0):.1f}`",
        "",
        "## Main Results",
        "",
        f"- `c1` ambig dense `{main_summary['c1']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, ambigmax dense `{main_summary['c1']['ambigmax'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c1', 0.0):+.4f}`",
        f"- `c2` ambig dense `{main_summary['c2']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, ambigmax dense `{main_summary['c2']['ambigmax'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c2', 0.0):+.4f}`",
        "",
        "## Fresh Rerun",
        "",
        f"- Regime: `{strongest_regime}`",
        f"- Ambiguity-aware dense: `{rerun_summary['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        f"- Ambiguity+max dense: `{rerun_summary['ambigmax'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        "",
        ...
        f"- Ambiguity+max sink/cache/cache-max/home probe test acc: `{probe_audit.get('ambigmax', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('ambigmax', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('ambigmax', {}).get('home_cache_max_state', 0.0):.3f}` / `{probe_audit.get('ambigmax', {}).get('home_hidden_state', 0.0):.3f}`",
        "",
        "## Conclusion",
        "",

...
ARMS = {
    ...
    "visit_taskgrad_half_hiconf_mutate": {
        "display": "VT-0.5 HCM",
        "growth": {
            "split_mode": "mutate",
    ...
    "visit_taskgrad_half_agree_mutate": {
        "display": "VT-0.5 CAG",
        "growth": {
            "split_mode": "mutate",
            "mutation_stage_index_min": 6,
    ...
    "visit_taskgrad_half_stagnate_mutate": {
        "display": "VT-0.5 SGM",
        "growth": {
            "split_mode": "mutate",
            "mutation_stage_index_min": 6,
...
def build_config(arm: str, regime: str, schedule: str) -> dict:
    ...
    return {
        ...
        "runtime": {
            "run_name": f"v30-{regime}-{arm}-32-{schedule}",
    ...
    for arm in ARMS:
        for regime, schedules in {
            ...
            for schedule in schedules:
                config_name = f"v30_{regime}_{arm}_32_{schedule}.yaml"
                run_name = f"v30-{regime}-{arm}-32-{schedule}"
                dump_yaml(build_config(arm, regime, schedule), CONFIGS / config_name)
                write_script(
                    SCRIPTS / f"train_v30_{regime}_{arm}_32_{schedule}.sh",
                    config_name,
                    run_name,
                    False,
                )
                if schedule == "l" and regime != "t2a":
                    write_script(
                        SCRIPTS / f"smoke_v30_{regime}_{arm}_32_{schedule}.sh",
                        config_name,
                        run_name,
                        True,
                    )

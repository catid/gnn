...
def build_config(arm: str, regime: str) -> dict:
    return {
        ...
        "runtime": {
            "run_name": f"v36-{regime}-{arm}-32-xl",
...
def main() -> None:
    ...
    for arm in ARMS:
        for regime in REGIMES:
            config_name = f"v36_{regime}_{arm}_32_xl.yaml"
            run_name = f"v36-{regime}-{arm}-32-xl"
            dump_yaml(build_config(arm, regime), CONFIGS / config_name)
            write_script(SCRIPTS / f"train_v36_{regime}_{arm}_32_xl.sh", config_name, run_name)

...
SUMMARY_PATH = REPORTS / "summary_metrics_v82.json"
REPORT_PATH = REPORTS / "final_report_v82_confidence_floor_gate.md"
PACK_DEFS_PATH = REPORTS / "v82_pack_definitions.json"
BASE_LR = 2.0e-4
EXPECTED_TRAIN_STEPS = {"p": 300, "m": 900, "l": 1350}
RUN_RE = re.compile(
    r"v82-collision-(?P<regime>c[12])-(?P<condition>ambig|confloor)-(?P<pair>[a-z0-9_]+)-32-(?P<schedule>p|m|l)(?:-(?P<tag>(?!s\d+$)[^-]+))?(?:-s(?P<seed>\d+))?$"
)
...
def latest_runs() -> list[tuple[dict[str, str], Path]]:
    ...
    for candidate in sorted(RUNS.glob("*-v82-*")):
...
def main() -> None:
    ...
    pilot_choices = {
        ...
        "confloor": choose_pilot(records, "confloor"),
    }
    ...
    for regime in ("c1", "c2"):
        ...
        confloor_records = [r for r in main_records if r["regime"] == regime and r["condition"] == "confloor"]
        main_summary[regime] = {
            "ambig": group_summary(ambig_records),
            "confloor": group_summary(confloor_records),
        }
        dense_gaps[regime] = (
            main_summary[regime]["confloor"].get("dense_mean", {}).get("mean", 0.0)
    ...
    rerun_summary = {
        ...
        "confloor": group_summary(
            [r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "confloor"]
        ),
    }

    probe_audit: dict[str, dict[str, Any]] = {}
    for condition in ("ambig", "confloor"):
    ...
    positive = (
        ...
        and rerun_summary["confloor"].get("dense_mean", {}).get("mean", 0.0)
        >= rerun_summary["ambig"].get("dense_mean", {}).get("mean", 0.0)
    )

    summary = {
        "experiment": "v82_confidence_floor_gate",
    ...
    report_lines = [
        ...
        f"- Confidence-floor LR multiplier: `{pilot_choices['confloor'].get('lr_multiplier', 0.0):.1f}`",
        "",
        "## Main Results",
        "",
        f"- `c1` ambig dense `{main_summary['c1']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, confloor dense `{main_summary['c1']['confloor'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c1', 0.0):+.4f}`",
        f"- `c2` ambig dense `{main_summary['c2']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, confloor dense `{main_summary['c2']['confloor'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c2', 0.0):+.4f}`",
        "",
        "## Fresh Rerun",
        "",
        f"- Regime: `{strongest_regime}`",
        f"- Ambiguity-aware dense: `{rerun_summary['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        f"- Confidence-floor dense: `{rerun_summary['confloor'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        "",
        ...
        f"- Confidence-floor sink/cache/cache-max/home probe test acc: `{probe_audit.get('confloor', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('confloor', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('confloor', {}).get('home_cache_max_state', 0.0):.3f}` / `{probe_audit.get('confloor', {}).get('home_hidden_state', 0.0):.3f}`",
        "",
        "## Conclusion",
        "",

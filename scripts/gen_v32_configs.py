...
ARMS = {
    ...
    "visit_taskgrad_half_agree_mutate_z00_m050": {
        "display": "VT-0.5 CAG-z0.0-m0.50",
        "kind": "mutation",
        "growth": {
            "split_mode": "mutate",
            "mutation_stage_index_min": 6,
            "mutation_selected_fraction": 1.0,
            "mutation_score_margin": 0.50,
            "mutation_min_visit_z": 0.0,
            "mutation_min_query_grad_z": 0.0,
        },
    },
    "visit_taskgrad_half_agree_mutate_z00_m075": {
        "display": "VT-0.5 CAG-z0.0-m0.75",
        "kind": "mutation",
        "growth": {
            "split_mode": "mutate",
            "mutation_stage_index_min": 6,
    ...
    "visit_taskgrad_half_agree_mutate_z00_m100": {
        "display": "VT-0.5 CAG-z0.0-m1.00",
        "kind": "mutation",
        "growth": {
            "split_mode": "mutate",
            "mutation_stage_index_min": 6,
            "mutation_selected_fraction": 1.0,
            "mutation_score_margin": 1.00,
            "mutation_min_visit_z": 0.0,
            "mutation_min_query_grad_z": 0.0,
        },
    },
...
def build_config(arm: str, regime: str, schedule: str) -> dict:
    return {
        ...
        "runtime": {
            "run_name": f"v32-{regime}-{arm}-32-{schedule}",
    ...
    for arm in ARMS:
        for regime, schedules in regime_schedules.items():
            for schedule in schedules:
                config_name = f"v32_{regime}_{arm}_32_{schedule}.yaml"
                run_name = f"v32-{regime}-{arm}-32-{schedule}"
                dump_yaml(build_config(arm, regime, schedule), CONFIGS / config_name)
                write_script(
                    SCRIPTS / f"train_v32_{regime}_{arm}_32_{schedule}.sh",
                    config_name,
                    run_name,
                    False,
                )
                if schedule == "l" and regime != "t2a":
                    write_script(
                        SCRIPTS / f"smoke_v32_{regime}_{arm}_32_{schedule}.sh",
                        config_name,
                        run_name,
                        True,
                    )

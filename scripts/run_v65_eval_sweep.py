...
RUN_RE = re.compile(
    r"v65-(?P<pack>collision|delay)-(?P<regime>[^-]+)-(?P<cond_a>[^-]+)(?:-(?P<cond_b>[^-]+))?-(?P<pair>[a-z0-9_]+)-32-(?P<schedule>p|m|l)(?:-(?P<tag>[^-]+))?-s(?P<seed>\d+)$"
)
...
def parse_run_name(name: str) -> dict[str, str] | None:
    ...
    if payload["pack"] == "delay":
        payload["condition"] = payload["cond_a"]
    else:
        payload["condition"] = f"{payload['cond_a']}-{payload['cond_b']}"
    return payload
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run v65 eval sweeps and probe collection.")
    parser.add_argument("--pack", choices=["collision", "delay"], required=True)

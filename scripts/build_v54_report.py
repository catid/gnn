...
SUMMARY_PATH = REPORTS / "summary_metrics_v54.json"
REPORT_PATH = REPORTS / "final_report_v54_stagelate_specialist_package_recheck.md"
...
PHASES = {
    ...
    "t2b_xl": {"regime": "t2b", "train_steps": 4590, "writers": [4, 8, 12, 14]},
    "t2c_xl": {"regime": "t2c", "train_steps": 4590, "writers": [6, 10, 14, 16]},
}
...
def phase_records(phase: str) -> list[dict[str, Any]]:
    ...
    for selector in SELECTORS:
        prefix = f"v54-{spec['regime']}-{selector}-32-xl"
...
def main() -> None:
    ...
    REPORT_PATH.write_text(
        "\n".join(
            [
                "# APSGNN v54: StageLate Specialist Package Recheck",
                "",
                "## What Changed",
                "",
                "v54 reruns the restored StageLate-0.5 specialist versus its lightweight sparse CAG package on the exact regimes the specialist currently owns: T1, T1r, T2b, and T2c.",
                "",
                "## Recommendation",
                "",
                f"Current overall winner on the fresh v54 specialist slice: {SELECTORS[final_recommendation]['label']}.",
                "",
                f"- Summary JSON: [summary_metrics_v54.json]({SUMMARY_PATH})",
                f"- Report: [final_report_v54_stagelate_specialist_package_recheck.md]({REPORT_PATH})",
                "",
            ]
        ),
        encoding="utf-8",

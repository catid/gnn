...
SUMMARY_PATH = REPORTS / "summary_metrics_v77.json"
REPORT_PATH = REPORTS / "final_report_v77_ambiguity_conditioned_max_gate.md"
PACK_DEFS_PATH = REPORTS / "v77_pack_definitions.json"
BASE_LR = 2.0e-4
EXPECTED_TRAIN_STEPS = {"p": 300, "m": 900, "l": 1350}
RUN_RE = re.compile(
    r"v77-collision-(?P<regime>c[12])-(?P<condition>ambig|ambigmaxgate)-(?P<pair>[a-z0-9_]+)-32-(?P<schedule>p|m|l)(?:-(?P<tag>(?!s\d+$)[^-]+))?(?:-s(?P<seed>\d+))?$"
)
...
def latest_runs() -> list[tuple[dict[str, str], Path]]:
    ...
    for candidate in sorted(RUNS.glob("*-v77-*")):
...
def main() -> None:
    ...
    pilot_choices = {
        ...
        "ambigmaxgate": choose_pilot(records, "ambigmaxgate"),
    }
    ...
    for regime in ("c1", "c2"):
        ...
        ambigmax_records = [r for r in main_records if r["regime"] == regime and r["condition"] == "ambigmaxgate"]
        main_summary[regime] = {
            "ambig": group_summary(ambig_records),
            "ambigmaxgate": group_summary(ambigmax_records),
        }
        dense_gaps[regime] = (
            main_summary[regime]["ambigmaxgate"].get("dense_mean", {}).get("mean", 0.0)
    ...
    rerun_summary = {
        ...
        "ambigmaxgate": group_summary(
            [r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "ambigmaxgate"]
        ),
    }

    probe_audit: dict[str, dict[str, Any]] = {}
    for condition in ("ambig", "ambigmaxgate"):
    ...
    positive = (
        ...
        and rerun_summary["ambigmaxgate"].get("dense_mean", {}).get("mean", 0.0)
        >= rerun_summary["ambig"].get("dense_mean", {}).get("mean", 0.0)
    )

    summary = {
        "experiment": "v77_ambiguity_conditioned_max_gate",
    ...
    report_lines = [
        ...
        f"- Ambiguity+max-gate LR multiplier: `{pilot_choices['ambigmaxgate'].get('lr_multiplier', 0.0):.1f}`",
        "",
        "## Main Results",
        "",
        f"- `c1` ambig dense `{main_summary['c1']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, ambigmaxgate dense `{main_summary['c1']['ambigmaxgate'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c1', 0.0):+.4f}`",
        f"- `c2` ambig dense `{main_summary['c2']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, ambigmaxgate dense `{main_summary['c2']['ambigmaxgate'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c2', 0.0):+.4f}`",
        "",
        "## Fresh Rerun",
        "",
        f"- Regime: `{strongest_regime}`",
        f"- Ambiguity-aware dense: `{rerun_summary['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        f"- Ambiguity+max-gate dense: `{rerun_summary['ambigmaxgate'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        "",
        ...
        f"- Ambiguity+max-gate sink/cache/cache-max/home probe test acc: `{probe_audit.get('ambigmaxgate', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('ambigmaxgate', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('ambigmaxgate', {}).get('home_cache_max_state', 0.0):.3f}` / `{probe_audit.get('ambigmaxgate', {}).get('home_hidden_state', 0.0):.3f}`",
        "",
        "## Conclusion",
        "",

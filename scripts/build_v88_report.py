...
SUMMARY_PATH = REPORTS / "summary_metrics_v88.json"
REPORT_PATH = REPORTS / "final_report_v88_collision_sharpened_summary_blend.md"
PACK_DEFS_PATH = REPORTS / "v88_pack_definitions.json"
BASE_LR = 2.0e-4
EXPECTED_TRAIN_STEPS = {"p": 300, "m": 900, "l": 1350}
RUN_RE = re.compile(
    r"v88-collision-(?P<regime>c[12])-(?P<condition>ambig|sharpblend)-(?P<pair>[a-z0-9_]+)-32-(?P<schedule>p|m|l)(?:-(?P<tag>(?!s\d+$)[^-]+))?(?:-s(?P<seed>\d+))?$"
)
...
def latest_runs() -> list[tuple[dict[str, str], Path]]:
    ...
    for candidate in sorted(RUNS.glob("*-v88-*")):
...
def main() -> None:
    ...
    pilot_choices = {
        ...
        "sharpblend": choose_pilot(records, "sharpblend"),
    }
    ...
    for regime in ("c1", "c2"):
        ...
        sharp_records = [r for r in main_records if r["regime"] == regime and r["condition"] == "sharpblend"]
        main_summary[regime] = {
            "ambig": group_summary(ambig_records),
            "sharpblend": group_summary(sharp_records),
        }
        dense_gaps[regime] = (
            main_summary[regime]["sharpblend"].get("dense_mean", {}).get("mean", 0.0)
    ...
    rerun_summary = {
        ...
        "sharpblend": group_summary(
            [r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "sharpblend"]
        ),
    }

    probe_audit: dict[str, dict[str, Any]] = {}
    for condition in ("ambig", "sharpblend"):
    ...
    positive = (
        ...
        and rerun_summary["sharpblend"].get("dense_mean", {}).get("mean", 0.0)
        >= rerun_summary["ambig"].get("dense_mean", {}).get("mean", 0.0)
    )

    summary = {
        "experiment": "v88_collision_sharpened_summary_blend",
    ...
    report_lines = [
        ...
        f"- Collision-sharpened-blend LR multiplier: `{pilot_choices['sharpblend'].get('lr_multiplier', 0.0):.1f}`",
        "",
        "## Main Results",
        "",
        f"- `c1` ambig dense `{main_summary['c1']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, sharpblend dense `{main_summary['c1']['sharpblend'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c1', 0.0):+.4f}`",
        f"- `c2` ambig dense `{main_summary['c2']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, sharpblend dense `{main_summary['c2']['sharpblend'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c2', 0.0):+.4f}`",
        "",
        "## Fresh Rerun",
        "",
        f"- Regime: `{strongest_regime}`",
        f"- Ambiguity-aware dense: `{rerun_summary['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        f"- Collision-sharpened-blend dense: `{rerun_summary['sharpblend'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        "",
        ...
        f"- Collision-sharpened-blend sink/cache/cache-max/home probe test acc: `{probe_audit.get('sharpblend', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('sharpblend', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('sharpblend', {}).get('home_cache_max_state', 0.0):.3f}` / `{probe_audit.get('sharpblend', {}).get('home_hidden_state', 0.0):.3f}`",
        "",
        "## Conclusion",
        "",

...
SUMMARY_PATH = REPORTS / "summary_metrics_v71.json"
REPORT_PATH = REPORTS / "final_report_v71_confidence_gated_output_blend.md"
PACK_DEFS_PATH = REPORTS / "v71_pack_definitions.json"
BASE_LR = 2.0e-4
EXPECTED_TRAIN_STEPS = {"p": 300, "m": 900, "l": 1350}
RUN_RE = re.compile(
    r"v71-collision-(?P<regime>c[12])-(?P<condition>mean|confblend)-(?P<pair>[a-z0-9_]+)-32-(?P<schedule>p|m|l)(?:-(?P<tag>(?!s\d+$)[^-]+))?(?:-s(?P<seed>\d+))?$"
)
...
def latest_runs() -> list[tuple[dict[str, str], Path]]:
    ...
    for candidate in sorted(RUNS.glob("*-v71-*")):
...
def main() -> None:
    ...
    pilot_choices = {
        ...
        "confblend": choose_pilot(records, "confblend"),
    }
    ...
    for regime in ("c1", "c2"):
        baseline = [r for r in main_records if r["regime"] == regime and r["condition"] == "mean"]
        readout = [r for r in main_records if r["regime"] == regime and r["condition"] == "confblend"]
        main_summary[regime] = {
            "mean": group_summary(baseline),
            "confblend": group_summary(readout),
        }
        dense_gaps[regime] = (
            main_summary[regime]["confblend"].get("dense_mean", {}).get("mean", 0.0)
    ...
    rerun_summary = {
        ...
        "confblend": group_summary([r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "confblend"]),
    }

    probe_audit = {}
    for condition in ("mean", "confblend"):
    ...
    positive = (
        ...
        and rerun_summary["confblend"].get("dense_mean", {}).get("mean", 0.0)
        >= rerun_summary["mean"].get("dense_mean", {}).get("mean", 0.0)
    )

    summary = {
        "experiment": "v71_confidence_gated_output_blend",
    ...
    report_lines = [
        ...
        f"- Confidence-blend LR multiplier: `{pilot_choices['confblend'].get('lr_multiplier', 0.0):.1f}`",
        "",
        "## Main Results",
        "",
        f"- `c1` mean dense `{main_summary['c1']['mean'].get('dense_mean', {}).get('mean', 0.0):.4f}`, confblend dense `{main_summary['c1']['confblend'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c1', 0.0):+.4f}`",
        f"- `c2` mean dense `{main_summary['c2']['mean'].get('dense_mean', {}).get('mean', 0.0):.4f}`, confblend dense `{main_summary['c2']['confblend'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c2', 0.0):+.4f}`",
        "",
        "## Fresh Rerun",
        "",
        f"- Regime: `{strongest_regime}`",
        f"- Mean dense: `{rerun_summary['mean'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        f"- Confidence-blend dense: `{rerun_summary['confblend'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        "",
        "## Probe Audit on C2 Best Checkpoints",
        "",
        f"- Mean sink/cache/home probe test acc: `{probe_audit.get('mean', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('mean', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('mean', {}).get('home_hidden_state', 0.0):.3f}`",
        f"- Confidence-blend sink/cache/home probe test acc: `{probe_audit.get('confblend', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('confblend', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('confblend', {}).get('home_hidden_state', 0.0):.3f}`",
        "",
        "## Conclusion",
        "",

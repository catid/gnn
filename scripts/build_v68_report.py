...
SUMMARY_PATH = REPORTS / "summary_metrics_v68.json"
REPORT_PATH = REPORTS / "final_report_v68_cache_output_readout.md"
PACK_DEFS_PATH = REPORTS / "v68_pack_definitions.json"
BASE_LR = 2.0e-4
EXPECTED_TRAIN_STEPS = {"p": 300, "m": 900, "l": 1350}
RUN_RE = re.compile(
    r"v68-collision-(?P<regime>c[12])-(?P<condition>baseline|readout)-(?P<pair>[a-z0-9_]+)-32-(?P<schedule>p|m|l)(?:-(?P<tag>(?!s\d+$)[^-]+))?(?:-s(?P<seed>\d+))?$"
)
...
def latest_runs() -> list[tuple[dict[str, str], Path]]:
    ...
    for candidate in sorted(RUNS.glob("*-v68-*")):
...
def main() -> None:
    ...
    pilot_choices = {
        ...
        "readout": choose_pilot(records, "readout"),
    }
    ...
    for regime in ("c1", "c2"):
        ...
        readout = [r for r in main_records if r["regime"] == regime and r["condition"] == "readout"]
        main_summary[regime] = {
            "baseline": group_summary(baseline),
            "readout": group_summary(readout),
        }
        dense_gaps[regime] = (
            main_summary[regime]["readout"].get("dense_mean", {}).get("mean", 0.0)
    ...
    rerun_summary = {
        ...
        "readout": group_summary([r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "readout"]),
    }

    probe_audit = {}
    for condition in ("baseline", "readout"):
    ...
    positive = (
        ...
        and rerun_summary["readout"].get("dense_mean", {}).get("mean", 0.0)
        >= rerun_summary["baseline"].get("dense_mean", {}).get("mean", 0.0)
    )

    summary = {
        "experiment": "v68_cache_output_readout_rescue",
    ...
    report_lines = [
        ...
        "## Pilot Choices",
        "",
        f"- Baseline LR multiplier: `{pilot_choices['baseline'].get('lr_multiplier', 0.0):.1f}`",
        f"- Readout LR multiplier: `{pilot_choices['readout'].get('lr_multiplier', 0.0):.1f}`",
        "",
        "## Main Results",
        "",
        f"- `c1` baseline dense `{main_summary['c1']['baseline'].get('dense_mean', {}).get('mean', 0.0):.4f}`, readout dense `{main_summary['c1']['readout'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c1', 0.0):+.4f}`",
        f"- `c2` baseline dense `{main_summary['c2']['baseline'].get('dense_mean', {}).get('mean', 0.0):.4f}`, readout dense `{main_summary['c2']['readout'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c2', 0.0):+.4f}`",
        "",
        "## Fresh Rerun",
        "",
        f"- Regime: `{strongest_regime}`",
        f"- Baseline dense: `{rerun_summary['baseline'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        f"- Readout dense: `{rerun_summary['readout'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        "",
        ...
        f"- Readout sink/cache/home probe test acc: `{probe_audit.get('readout', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('readout', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('readout', {}).get('home_hidden_state', 0.0):.3f}`",
        "",
        "## Conclusion",
        "",

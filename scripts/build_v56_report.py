...
SUMMARY_PATH = REPORTS / "summary_metrics_v56.json"
REPORT_PATH = REPORTS / "final_report_v56_stagelate_package_slice_recheck.md"

SELECTORS = {
    "stageadaptive_late_half": {"label": "StageLate-0.5"},
    "stageadaptive_late_half_agree_mutate_z00_m075_f0125": {"label": "StageLate-0.5 CAG-z0.0-m0.75-f0.125"},
}

PHASES = {
    "fresh_t1_xl": {"prefixes": ["v56-t1"], "train_steps": 4590, "writers": [4, 8, 12, 14]},
    "fresh_t1r_xl": {"prefixes": ["v56-t1r"], "train_steps": 4590, "writers": [4, 8, 12, 14]},
    "fresh_t2c_xl": {"prefixes": ["v56-t2c"], "train_steps": 4590, "writers": [6, 10, 14, 16]},
    "pooled_t1_xl": {"prefixes": ["v54-t1", "v56-t1"], "train_steps": 4590, "writers": [4, 8, 12, 14]},
    "pooled_t1r_xl": {"prefixes": ["v54-t1r", "v56-t1r"], "train_steps": 4590, "writers": [4, 8, 12, 14]},
    "pooled_t2c_xl": {"prefixes": ["v54-t2c", "v56-t2c"], "train_steps": 4590, "writers": [6, 10, 14, 16]},
}
...
def main() -> None:
    ...
    final_recommendation = max(final_score, key=final_score.get) if final_score else "stageadaptive_late_half"
    payload = {
    ...
    REPORT_PATH.write_text(
        "\n".join(
            [
                "# APSGNN v56: StageLate Package Slice Recheck",
                "",
                "## What Changed",
                "",
                "v56 reruns only the StageLate regimes that ever favored the lightweight sparse CAG package: T1, T1r, and T2c. It then pools those fresh runs with the earlier v54 package-friendly slice.",
                "",
                "## Recommendation",
                "",
                f"Current overall winner on the fresh+pooled package-friendly slice: {SELECTORS[final_recommendation]['label']}.",
                "",
                f"- Summary JSON: [summary_metrics_v56.json]({SUMMARY_PATH})",
                f"- Report: [final_report_v56_stagelate_package_slice_recheck.md]({REPORT_PATH})",
                "",
            ]
        ),
        encoding="utf-8",

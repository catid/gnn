...
SUMMARY_PATH = REPORTS / "summary_metrics_v58.json"
REPORT_PATH = REPORTS / "final_report_v58_v_vs_vt_half_recheck.md"
...
def main() -> None:
    ...
    REPORT_PATH.write_text(
        "\n".join(
            [
                "# APSGNN v58: Final V vs VT-0.5 Recheck",
                "",
                "## What Changed",
                "",
                "v58 reruns the six-regime matched selector comparison on fresh same-seed pairs after v57 collapsed the StageLate regime-keyed rule. It directly rechecks the last old global-default challenger, V, against VT-0.5 under the same fresh XL structure.",
                "",
                "## Recommendation",
                "",
                f"Fresh rule winner: `{fresh_rule_recommendation}` ({RULES[fresh_rule_recommendation]['label']}).",
                "",
                f"- Summary JSON: [summary_metrics_v58.json]({SUMMARY_PATH})",
                f"- Report: [final_report_v58_v_vs_vt_half_recheck.md]({REPORT_PATH})",
                "",
            ]
        ),
        encoding="utf-8",

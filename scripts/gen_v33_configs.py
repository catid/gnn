    ...
    return {
        ...
        "train": {
            **copy.deepcopy(TRAIN_BASE),
            "train_steps": SCHEDULES["xl"]["train_steps"],
            "eval_interval": SCHEDULES["xl"]["eval_interval"],
            "log_interval": 50,
            "save_interval": SCHEDULES["xl"]["save_interval"],
            "first_hop_teacher_force_anneal_steps": SCHEDULES["xl"]["anneal_steps"],
        },
        "runtime": {
            "run_name": f"v33-{regime}-{arm}-32-xl",
            "notes": f"selector={ARMS[arm]['display']}",
        },
        "growth": {
            **copy.deepcopy(GROWTH_BASE),
            **copy.deepcopy(ARMS[arm]["growth"]),
            "stage_steps": copy.deepcopy(SCHEDULES["xl"]["stage_steps"]),
        },
    }
...
def main() -> None:
    ...
    for arm in ARMS:
        for regime in REGIMES:
            config_name = f"v33_{regime}_{arm}_32_xl.yaml"
            run_name = f"v33-{regime}-{arm}-32-xl"
            dump_yaml(build_config(arm, regime), CONFIGS / config_name)
            write_script(SCRIPTS / f"train_v33_{regime}_{arm}_32_xl.sh", config_name, run_name)

...
SUMMARY_PATH = REPORTS / "summary_metrics_v78.json"
REPORT_PATH = REPORTS / "final_report_v78_ambiguity_feature_output_modulation.md"
PACK_DEFS_PATH = REPORTS / "v78_pack_definitions.json"
BASE_LR = 2.0e-4
EXPECTED_TRAIN_STEPS = {"p": 300, "m": 900, "l": 1350}
RUN_RE = re.compile(
    r"v78-collision-(?P<regime>c[12])-(?P<condition>ambig|ambigfilm)-(?P<pair>[a-z0-9_]+)-32-(?P<schedule>p|m|l)(?:-(?P<tag>(?!s\d+$)[^-]+))?(?:-s(?P<seed>\d+))?$"
)
...
def latest_runs() -> list[tuple[dict[str, str], Path]]:
    ...
    for candidate in sorted(RUNS.glob("*-v78-*")):
...
def main() -> None:
    ...
    pilot_choices = {
        ...
        "ambigfilm": choose_pilot(records, "ambigfilm"),
    }
    ...
    for regime in ("c1", "c2"):
        ...
        ambigfilm_records = [r for r in main_records if r["regime"] == regime and r["condition"] == "ambigfilm"]
        main_summary[regime] = {
            "ambig": group_summary(ambig_records),
            "ambigfilm": group_summary(ambigfilm_records),
        }
        dense_gaps[regime] = (
            main_summary[regime]["ambigfilm"].get("dense_mean", {}).get("mean", 0.0)
    ...
    rerun_summary = {
        ...
        "ambigfilm": group_summary(
            [r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "ambigfilm"]
        ),
    }

    probe_audit: dict[str, dict[str, Any]] = {}
    for condition in ("ambig", "ambigfilm"):
    ...
    positive = (
        ...
        and rerun_summary["ambigfilm"].get("dense_mean", {}).get("mean", 0.0)
        >= rerun_summary["ambig"].get("dense_mean", {}).get("mean", 0.0)
    )

    summary = {
        "experiment": "v78_ambiguity_feature_output_modulation",
    ...
    report_lines = [
        ...
        f"- Ambiguity-feature-modulated LR multiplier: `{pilot_choices['ambigfilm'].get('lr_multiplier', 0.0):.1f}`",
        "",
        "## Main Results",
        "",
        f"- `c1` ambig dense `{main_summary['c1']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, ambigfilm dense `{main_summary['c1']['ambigfilm'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c1', 0.0):+.4f}`",
        f"- `c2` ambig dense `{main_summary['c2']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, ambigfilm dense `{main_summary['c2']['ambigfilm'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c2', 0.0):+.4f}`",
        "",
        "## Fresh Rerun",
        "",
        f"- Regime: `{strongest_regime}`",
        f"- Ambiguity-aware dense: `{rerun_summary['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        f"- Ambiguity-feature-modulated dense: `{rerun_summary['ambigfilm'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        "",
        ...
        f"- Ambiguity-feature-modulated sink/cache/cache-max/home probe test acc: `{probe_audit.get('ambigfilm', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('ambigfilm', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('ambigfilm', {}).get('home_cache_max_state', 0.0):.3f}` / `{probe_audit.get('ambigfilm', {}).get('home_hidden_state', 0.0):.3f}`",
        "",
        "## Conclusion",
        "",

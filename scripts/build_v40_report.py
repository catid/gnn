...
SUMMARY_PATH = REPORTS / "summary_metrics_v40.json"
REPORT_PATH = REPORTS / "final_report_v40_sparse_transfer_fraction_refinement.md"

SELECTORS = {
    "visit_taskgrad_half": {"label": "VT-0.5"},
    "visit_taskgrad_half_agree_mutate_z00_m075_f0125": {"label": "VT-0.5 CAG-z0.0-m0.75-f0.125"},
...
PHASES = {
    "t1_xl": {
        ...
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "title": "T1-XL Fresh Transfer Refinement",
    },
    "t1r_xl": {
        "regime": "t1r",
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "title": "T1-XL Rerun Transfer Refinement",
    },
}
...
def phase_records(phase: str) -> list[dict[str, Any]]:
    ...
    for selector in SELECTORS:
        prefix = f"v40-{spec['regime']}-{selector}-32-xl"
...
def main() -> None:
    ...
    for phase, summary in phase_summaries.items():
        plot_phase(summary, phase, REPORTS / f"v40_{phase}_summary.png")
    ...
    lines = [
        "# APSGNN v40: Sparse Transfer Fraction Refinement",
        "",
        "## What Changed",
        "",
        "v40 narrows the live question from v39 to a single transfer-specialist knob: sparse mutation fraction.",
        "It compares mutation-free `VT-0.5` against `f0.125` and `f0.25` on fresh transfer and rerun transfer only.",
        "",
        "## Outputs",
        "",
        f"- Summary JSON: [summary_metrics_v40.json]({SUMMARY_PATH})",
        f"- Report: [final_report_v40_sparse_transfer_fraction_refinement.md]({REPORT_PATH})",
        "",
    ]

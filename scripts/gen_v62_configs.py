...
SCHEDULES = {
    ...
    "xl": {
        "display": "XL",
        "train_steps": 3780,
        "stage_steps": [99, 99, 123, 123, 149, 182, 3005],
        "eval_interval": 210,
        "save_interval": 210,
        "anneal_steps": 1000,
    },
...
REPRESENTATIVE_PILOT_REGIMES = ["core", "t1", "t2a", "hmix"]
...
def build_config(arm: str, regime: str, schedule: str) -> dict:
    ...
    return {
        ...
        "runtime": {
            "run_name": f"v62-{regime}-{arm}-32-{schedule}",
            "notes": (
    ...
    for arm in ARMS:
        for regime in REGIMES:
            for schedule in SCHEDULES:
                config_name = f"v62_{regime}_{arm}_32_{schedule}.yaml"
                run_name = f"v62-{regime}-{arm}-32-{schedule}"
                dump_yaml(build_config(arm, regime, schedule), CONFIGS / config_name)
                write_script(SCRIPTS / f"train_v62_{regime}_{arm}_32_{schedule}.sh", config_name, run_name)

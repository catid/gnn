...
SUMMARY_PATH = REPORTS / "summary_metrics_v59.json"
REPORT_PATH = REPORTS / "final_report_v59_v_vs_vt_half_expansion.md"
...
PHASES = {
    ...
    "pooled_core_xl": {
        "prefixes": ["v58-core", "v59-core"],
        "train_steps": 4590,
        "writers": [2, 6, 10],
        "family": "home",
    },
    "pooled_t1_xl": {
        "prefixes": ["v58-t1", "v59-t1"],
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "family": "transfer",
    },
    "pooled_t1r_xl": {
        "prefixes": ["v58-t1r", "v59-t1r"],
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "family": "transfer",
    },
    "pooled_t2a_xl": {
        "prefixes": ["v58-t2a", "v59-t2a"],
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "family": "ingress",
    },
    "pooled_t2b_xl": {
        "prefixes": ["v58-t2b", "v59-t2b"],
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "family": "non_ingress_stress",
    },
    "pooled_t2c_xl": {
        "prefixes": ["v58-t2c", "v59-t2c"],
        "train_steps": 4590,
        "writers": [6, 10, 14, 16],
        "family": "non_ingress_stress",
    },
...
def main() -> None:
    ...
    REPORT_PATH.write_text(
        "\n".join(
            [
                "# APSGNN v59: V vs VT-0.5 Expansion",
                "",
                "## What Changed",
                "",
                "v59 adds a second fresh same-seed six-regime matched pair after v58 reopened the default-selector question. It reports both fresh-v59 results and the pooled v58+v59 same-structure aggregate.",
                "",
                "## Recommendation",
                "",
                f"Fresh rule winner: `{fresh_rule_recommendation}` ({RULES[fresh_rule_recommendation]['label']}).",
                f"Pooled v58+v59 rule winner: `{pooled_rule_recommendation}` ({RULES[pooled_rule_recommendation]['label']}).",
                "",
                f"- Summary JSON: [summary_metrics_v59.json]({SUMMARY_PATH})",
                f"- Report: [final_report_v59_v_vs_vt_half_expansion.md]({REPORT_PATH})",
                "",
            ]
        ),
        encoding="utf-8",

...
def build_config(selector: str, regime: str, schedule: str) -> dict:
    ...
    return {
        ...
        "runtime": {
            "run_name": f"v25-{regime}-{selector}-32-{schedule}",
        },
        "growth": {
    ...
    for selector in SELECTORS:
        for regime, schedules in {
            ...
            for schedule in schedules:
                config_name = f"v25_{regime}_{selector}_32_{schedule}.yaml"
                dump_yaml(build_config(selector, regime, schedule), CONFIGS / config_name)
                write_script(
                    SCRIPTS / f"train_v25_{regime}_{selector}_32_{schedule}.sh",
                    config_name,
                    f"v25-{regime}-{selector}-32-{schedule}",
                    smoke=False,
                )
                if schedule == "l":
                    write_script(
                        SCRIPTS / f"smoke_v25_{regime}_{selector}_32_{schedule}.sh",
                        config_name,
                        f"v25-{regime}-{selector}-32-{schedule}",
                        smoke=True,
                    )

        config_name = f"v25_t2a_{selector}_32_xl.yaml"
        dump_yaml(build_config(selector, "t2a", "xl"), CONFIGS / config_name)
        write_script(
            SCRIPTS / f"train_v25_t2a_{selector}_32_xl.sh",
            config_name,
            f"v25-t2a-{selector}-32-xl",
            smoke=False,
        )

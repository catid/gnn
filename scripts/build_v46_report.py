...
SUMMARY_PATH = REPORTS / "summary_metrics_v46.json"
REPORT_PATH = REPORTS / "final_report_v46_stagelate_package_disambiguation.md"
...
def phase_records(phase: str) -> list[dict[str, Any]]:
    ...
    for selector in SELECTORS:
        prefix = f"v46-{spec['regime']}-{selector}-32-xl"
...
def main() -> None:
    ...
    REPORT_PATH.write_text(
        "\n".join(
            [
                "# APSGNN v46: StageLate Specialist Disambiguation",
                "",
                "## What Changed",
                "",
                "v46 directly compares VT-0.5, StageLate-0.5, and StageLate-0.5 CAG-z0.0-m0.75-f0.125 on transfer-only regimes to separate selector effects from package effects.",
                "",
                f"- Summary JSON: [summary_metrics_v46.json]({SUMMARY_PATH})",
                f"- Report: [final_report_v46_stagelate_package_disambiguation.md]({REPORT_PATH})",
                "",
            ]
        ),
        encoding="utf-8",

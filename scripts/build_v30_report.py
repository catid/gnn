...
SUMMARY_PATH = REPORTS / "summary_metrics_v30.json"
REPORT_PATH = REPORTS / "final_report_v30_mutation_revisit.md"

SELECTORS = {
    "visit_taskgrad_half": {"label": "VT-0.5", "kind": "baseline"},
    "visit_taskgrad_half_hiconf_mutate": {"label": "VT-0.5 HCM", "kind": "mutation"},
    "visit_taskgrad_half_agree_mutate": {"label": "VT-0.5 CAG", "kind": "mutation"},
    "visit_taskgrad_half_stagnate_mutate": {"label": "VT-0.5 SGM", "kind": "mutation"},
}
...
def phase_records(phase: str) -> list[dict[str, Any]]:
    ...
    for selector in spec["selectors"]:
        prefix = f"v30-{spec['regime']}-{selector}-32-{spec['schedule']}"
...
def main() -> None:
    ...
    promoted_label = SELECTORS[promoted_mutation]["label"] if promoted_mutation else None

    final_score: dict[str, float] = {}
    finalists = ["visit_taskgrad_half"]
    if promoted_mutation:
        finalists.append(promoted_mutation)
    for selector in finalists:
        total = 0.0
    ...
    for phase in phase_summaries:
        plot_phase(phase_summaries[phase], phase, REPORTS / f"v30_{phase}_summary.png")
    ...
    lines = [
        ...
        "## Completed Runs",
        "",
        run_table(all_records) if all_records else "_No completed substantive runs yet._",
        "",
    ]
    if core_l_summary:
        lines.extend(
            [
                ...
                "",
                f"Promoted mutation arm: `{promoted_label}`" if promoted_label else "No mutation arm has completed enough runs to promote.",
                "",
            ]
        )
    for phase in ("core_xl", "t1_l", "t1_xl", "t2a_xl"):
        ...
        if not summary:
            continue
        lines.extend(["## " + PHASES[phase]["title"], "", phase_table(summary, phase), ""])

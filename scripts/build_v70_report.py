...
SUMMARY_PATH = REPORTS / "summary_metrics_v70.json"
REPORT_PATH = REPORTS / "final_report_v70_dual_output_readout.md"
PACK_DEFS_PATH = REPORTS / "v70_pack_definitions.json"
BASE_LR = 2.0e-4
EXPECTED_TRAIN_STEPS = {"p": 300, "m": 900, "l": 1350}
RUN_RE = re.compile(
    r"v70-collision-(?P<regime>c[12])-(?P<condition>mean|dual)-(?P<pair>[a-z0-9_]+)-32-(?P<schedule>p|m|l)(?:-(?P<tag>(?!s\d+$)[^-]+))?(?:-s(?P<seed>\d+))?$"
)
...
def latest_runs() -> list[tuple[dict[str, str], Path]]:
    ...
    for candidate in sorted(RUNS.glob("*-v70-*")):
...
def main() -> None:
    ...
    pilot_choices = {
        ...
        "dual": choose_pilot(records, "dual"),
    }
    ...
    for regime in ("c1", "c2"):
        ...
        readout = [r for r in main_records if r["regime"] == regime and r["condition"] == "dual"]
        main_summary[regime] = {
            "mean": group_summary(baseline),
            "dual": group_summary(readout),
        }
        dense_gaps[regime] = (
            main_summary[regime]["dual"].get("dense_mean", {}).get("mean", 0.0)
    ...
    rerun_summary = {
        ...
        "dual": group_summary([r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "dual"]),
    }

    probe_audit = {}
    for condition in ("mean", "dual"):
    ...
    positive = (
        ...
        and rerun_summary["dual"].get("dense_mean", {}).get("mean", 0.0)
        >= rerun_summary["mean"].get("dense_mean", {}).get("mean", 0.0)
    )

    summary = {
        "experiment": "v70_dual_output_readout",
    ...
    report_lines = [
        ...
        f"- Dual-readout LR multiplier: `{pilot_choices['dual'].get('lr_multiplier', 0.0):.1f}`",
        "",
        "## Main Results",
        "",
        f"- `c1` mean dense `{main_summary['c1']['mean'].get('dense_mean', {}).get('mean', 0.0):.4f}`, dual dense `{main_summary['c1']['dual'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c1', 0.0):+.4f}`",
        f"- `c2` mean dense `{main_summary['c2']['mean'].get('dense_mean', {}).get('mean', 0.0):.4f}`, dual dense `{main_summary['c2']['dual'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c2', 0.0):+.4f}`",
        "",
        "## Fresh Rerun",
        "",
        f"- Regime: `{strongest_regime}`",
        f"- Mean dense: `{rerun_summary['mean'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        f"- Dual dense: `{rerun_summary['dual'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        "",
        ...
        f"- Dual sink/cache/home probe test acc: `{probe_audit.get('dual', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('dual', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('dual', {}).get('home_hidden_state', 0.0):.3f}`",
        "",
        "## Conclusion",
        "",

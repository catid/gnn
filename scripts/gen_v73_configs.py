...
MODEL_BASE = {
    ...
    "cache_output_summary_source": "mean",
...
CONDITIONS = {
    ...
    "disp": {
        "display": "DispersionAugmentedReadout",
        "model": {
            "enable_cache": True,
            "cache_output_summary_readout": True,
            "cache_output_summary_source": "dispersion_augmented",
            "cache_output_dispersion_scale": 1.0,
        },
    },
}
...
def base_config() -> dict:
    return {
        ...
        "runtime": {
            ...
            "report_root": "reports",
            "run_name": "",
            "notes": "v73 dispersion-augmented output readout",
        },
...
def pack_definitions() -> dict:
    return {
        ...
        "schedules": SCHEDULES,
        "promotion_rule": "Positive only if the dispersion-augmented readout improves C2 dense_mean in the main seeds and does not lose the fresh rerun.",
    }
    ...
    for regime in COLLISION_REGIMES:
        for condition in CONDITIONS:
            for schedule in SCHEDULES:
                cfg = build_collision_config(regime, schedule, condition=condition)
                cfg_path = CONFIGS / config_filename(regime, condition, schedule)
                write_yaml(cfg_path, cfg)
                write_train_script(SCRIPTS / script_filename(regime, condition, schedule), cfg_path)
    PACK_DEFS_PATH = REPORTS / "v73_pack_definitions.json"
    PACK_DEFS_PATH.write_text(json.dumps(pack_definitions(), indent=2, sort_keys=True) + "\n", encoding="utf-8")

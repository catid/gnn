...
SUMMARY_PATH = REPORTS / "summary_metrics_v47.json"
REPORT_PATH = REPORTS / "final_report_v47_lateswitch_weight_neighborhood.md"

SELECTORS = {
    "visit_taskgrad_half": {"label": "VT-0.5"},
    "stageadaptive_late_quarter": {"label": "StageLate-0.25"},
    "stageadaptive_late_0375": {"label": "StageLate-0.375"},
...
def phase_records(phase: str) -> list[dict[str, Any]]:
    ...
    for selector in SELECTORS:
        prefix = f"v47-{spec['regime']}-{selector}-32-xl"
...
def main() -> None:
    ...
    REPORT_PATH.write_text(
        "\n".join(
            [
                "# APSGNN v47: Late-Switch Weight Neighborhood",
                "",
                "## What Changed",
                "",
                "v47 narrows the post-v46 question to milder late-stage switch weights, comparing VT-0.5 against StageLate-0.25, StageLate-0.375, and StageLate-0.5 on fresh matched transfer-only regimes.",
                "",
                f"- Summary JSON: [summary_metrics_v47.json]({SUMMARY_PATH})",
                f"- Report: [final_report_v47_lateswitch_weight_neighborhood.md]({REPORT_PATH})",
                "",
            ]
        ),
        encoding="utf-8",

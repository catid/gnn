...
SUMMARY_PATH = REPORTS / "summary_metrics_v43.json"
REPORT_PATH = REPORTS / "final_report_v43_matched_regime_rule_expansion.md"
VERSIONS = ("v42", "v43")
...
PHASES = {
    "core_xl": {
        ...
        "train_steps": 4590,
        "writers": [2, 6, 10],
        "title": "Core-XL Matched Expansion",
    },
    "t1_xl": {
        "regime": "t1",
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "title": "T1-XL Matched Expansion",
    },
    "t1r_xl": {
        "regime": "t1r",
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "title": "T1r-XL Matched Expansion",
    },
    "t2a_xl": {
        "regime": "t2a",
        "train_steps": 4590,
        "writers": [4, 8, 12, 14],
        "title": "T2a-XL Matched Expansion",
    },
}
...
def summarize_run(run_dir: Path, selector: str, phase: str, seed: int, version: str) -> dict[str, Any]:
    spec = PHASES[phase]
...
def phase_records(phase: str) -> list[dict[str, Any]]:
    ...
    for selector in SELECTORS:
        for version in VERSIONS:
            prefix = f"{version}-{spec['regime']}-{selector}-32-xl"
            for run_dir, seed in latest_runs(prefix):
                if not is_complete_run(run_dir, spec["train_steps"]):
                    continue
                records.append(summarize_run(run_dir, selector, phase, seed, version))
    return records
...
def main() -> None:
    ...
    for phase, summary in phase_summaries.items():
        plot_phase(summary, phase, REPORTS / f"v43_{phase}_summary.png")
    ...
    if final_recommendation == "visit_taskgrad_half":
        recommendation = "Expanded matched evidence favors a single global default: VT-0.5."
    else:
        recommendation = "Expanded matched evidence still favors a regime-dependent rule with StageLate-0.5 as the transfer/stress specialist."

    REPORT_PATH.write_text(
        "\n".join(
            [
                "# APSGNN v43: Matched Regime Rule Expansion",
                "",
                "## What Changed",
                "",
                "v43 extends v42 with an additional fresh same-seed pair and aggregates both rounds into one matched four-seed confirmation set.",
                "",
                "## Recommendation",
                "",
                recommendation,
                "",
                f"- Summary JSON: [summary_metrics_v43.json]({SUMMARY_PATH})",
                f"- Report: [final_report_v43_matched_regime_rule_expansion.md]({REPORT_PATH})",
                "",
            ]
        ),
        encoding="utf-8",

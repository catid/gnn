...
SUMMARY_PATH = REPORTS / "summary_metrics_v83.json"
REPORT_PATH = REPORTS / "final_report_v83_collision_switch_gate.md"
PACK_DEFS_PATH = REPORTS / "v83_pack_definitions.json"
BASE_LR = 2.0e-4
EXPECTED_TRAIN_STEPS = {"p": 300, "m": 900, "l": 1350}
RUN_RE = re.compile(
    r"v83-collision-(?P<regime>c[12])-(?P<condition>ambig|collswitch)-(?P<pair>[a-z0-9_]+)-32-(?P<schedule>p|m|l)(?:-(?P<tag>(?!s\d+$)[^-]+))?(?:-s(?P<seed>\d+))?$"
)
...
def latest_runs() -> list[tuple[dict[str, str], Path]]:
    ...
    for candidate in sorted(RUNS.glob("*-v83-*")):
...
def main() -> None:
    ...
    pilot_choices = {
        ...
        "collswitch": choose_pilot(records, "collswitch"),
    }
    ...
    for regime in ("c1", "c2"):
        ...
        collswitch_records = [r for r in main_records if r["regime"] == regime and r["condition"] == "collswitch"]
        main_summary[regime] = {
            "ambig": group_summary(ambig_records),
            "collswitch": group_summary(collswitch_records),
        }
        dense_gaps[regime] = (
            main_summary[regime]["collswitch"].get("dense_mean", {}).get("mean", 0.0)
    ...
    rerun_summary = {
        ...
        "collswitch": group_summary(
            [r for r in rerun_records if r["regime"] == strongest_regime and r["condition"] == "collswitch"]
        ),
    }

    probe_audit: dict[str, dict[str, Any]] = {}
    for condition in ("ambig", "collswitch"):
    ...
    positive = (
        ...
        and rerun_summary["collswitch"].get("dense_mean", {}).get("mean", 0.0)
    ...
    report_lines = [
        ...
        f"- Collision-switch LR multiplier: `{pilot_choices['collswitch'].get('lr_multiplier', 0.0):.1f}`",
        "",
        "## Main Results",
        "",
        f"- `c1` ambig dense `{main_summary['c1']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, collswitch dense `{main_summary['c1']['collswitch'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c1', 0.0):+.4f}`",
        f"- `c2` ambig dense `{main_summary['c2']['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`, collswitch dense `{main_summary['c2']['collswitch'].get('dense_mean', {}).get('mean', 0.0):.4f}`, gap `{dense_gaps.get('c2', 0.0):+.4f}`",
        "",
        "## Fresh Rerun",
        "",
        f"- Regime: `{strongest_regime}`",
        f"- Ambiguity-aware dense: `{rerun_summary['ambig'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        f"- Collision-switch dense: `{rerun_summary['collswitch'].get('dense_mean', {}).get('mean', 0.0):.4f}`",
        "",
        ...
        f"- Collision-switch sink/cache/cache-max/home probe test acc: `{probe_audit.get('collswitch', {}).get('final_sink_state', 0.0):.3f}` / `{probe_audit.get('collswitch', {}).get('home_cache_mean_state', 0.0):.3f}` / `{probe_audit.get('collswitch', {}).get('home_cache_max_state', 0.0):.3f}` / `{probe_audit.get('collswitch', {}).get('home_hidden_state', 0.0):.3f}`",
        "",
        "## Conclusion",
        "",

...
ARMS = {
    "visit_taskgrad_half": {
        "display": "VT-0.5",
        "growth": {"split_mode": "clone"},
    },
    "visit_taskgrad_half_agree_mutate_z00_m075_f0125": {
        "display": "VT-0.5 CAG-z0.0-m0.75-f0.125",
        "growth": {
            "split_mode": "mutate",
            "mutation_stage_index_min": 6,
            "mutation_selected_fraction": 0.125,
            "mutation_score_margin": 0.75,
            "mutation_min_visit_z": 0.0,
            "mutation_min_query_grad_z": 0.0,
        },
    },
    "visit_taskgrad_half_agree_mutate_z00_m075_f025": {
        "display": "VT-0.5 CAG-z0.0-m0.75-f0.25",
        "growth": {
            "split_mode": "mutate",
            "mutation_stage_index_min": 6,
            "mutation_selected_fraction": 0.25,
            "mutation_score_margin": 0.75,
            "mutation_min_visit_z": 0.0,
            "mutation_min_query_grad_z": 0.0,
        },
    },
    "visit_taskgrad_half_agree_mutate_z00_m075_f0375": {
        "display": "VT-0.5 CAG-z0.0-m0.75-f0.375",
        "growth": {
            "split_mode": "mutate",
            "mutation_stage_index_min": 6,
            "mutation_selected_fraction": 0.375,
            "mutation_score_margin": 0.75,
            "mutation_min_visit_z": 0.0,
            "mutation_min_query_grad_z": 0.0,
        },
...
def write_script(path: Path, config_name: str, run_name: str) -> None:
    ...
    path.write_text(
        f"#!/usr/bin/env bash\nCONFIG=configs/{config_name} DEFAULT_SEED=1234 DEFAULT_RUN_NAME={run_name} exec \"$(dirname \"$0\")/_train_wrapper_single_gpu.sh\"\n",
        encoding="utf-8",
    )
    path.chmod(0o755)
    ...
    return {
        ...
        "train": {
            **copy.deepcopy(TRAIN_BASE),
            "train_steps": SCHEDULES[schedule]["train_steps"],
            "eval_interval": SCHEDULES[schedule]["eval_interval"],
            "log_interval": 50,
            "save_interval": SCHEDULES[schedule]["save_interval"],
            "first_hop_teacher_force_anneal_steps": SCHEDULES[schedule]["anneal_steps"],
        },
        "runtime": {
            "run_name": f"v37-{regime}-{arm}-32-{schedule}",
            "notes": f"selector={ARMS[arm]['display']}",
        },
        "growth": {
            **copy.deepcopy(GROWTH_BASE),
            **copy.deepcopy(ARMS[arm]["growth"]),
            "stage_steps": copy.deepcopy(SCHEDULES[schedule]["stage_steps"]),
        },
    }
...
def main() -> None:
    ...
    for arm in ARMS:
        for regime, schedules in regime_schedules.items():
            for schedule in schedules:
                config_name = f"v37_{regime}_{arm}_32_{schedule}.yaml"
                run_name = f"v37-{regime}-{arm}-32-{schedule}"
                dump_yaml(build_config(arm, regime, schedule), CONFIGS / config_name)
                write_script(SCRIPTS / f"train_v37_{regime}_{arm}_32_{schedule}.sh", config_name, run_name)

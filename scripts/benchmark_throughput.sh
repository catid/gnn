...
python -m apsgnn.train --config configs/throughput_1gpu.yaml --run-name throughput-1gpu --benchmark-only
...
torchrun --standalone --nproc_per_node="$nproc" -m apsgnn.train --config configs/throughput_4gpu.yaml --run-name throughput-multigpu --benchmark-only

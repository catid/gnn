...
def test_v57_generator_emits_all_rule_regimes():
    module = load_module("gen_v58_configs", "scripts/gen_v58_configs.py")
...
def test_v57_eval_completion_filter(tmp_path):
    module = load_module("run_v58_eval_sweep", "scripts/run_v58_eval_sweep.py")
...
def test_v57_report_builder_tracks_rules_and_scopes():
    module = load_module("build_v58_report", "scripts/build_v58_report.py")
    assert set(module.SELECTORS) == {"visitonly", "visit_taskgrad_half"}
    assert set(module.RULES) == {"single_selector_v", "single_selector_vt_half"}

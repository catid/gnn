...
def test_v78_generator_emits_ambig_and_ambigfilm_pack() -> None:
    module = load_module("gen_v78_configs", "scripts/gen_v78_configs.py")
    assert set(module.COLLISION_REGIMES) == {"c1", "c2"}
    assert set(module.CONDITIONS) == {"ambig", "ambigfilm"}
    packs = module.pack_definitions()
    assert packs["experiment"] == "v78_ambiguity_feature_output_modulation"


def test_v78_configs_select_sources_cleanly() -> None:
    module = load_module("gen_v78_cfg", "scripts/gen_v78_configs.py")
    ambig_cfg = module.build_collision_config("c2", "m", condition="ambig")
    ambigfilm_cfg = module.build_collision_config("c2", "m", condition="ambigfilm")
    assert ambig_cfg["model"]["cache_output_summary_source"] == "ambiguity_gate"
    assert ambigfilm_cfg["model"]["cache_output_summary_source"] == "ambiguity_feature_modulation"
    assert ambigfilm_cfg["model"]["cache_output_gate_feature_scale"] == 1.0
    assert ambigfilm_cfg["model"]["cache_output_feature_modulation_scale"] == 1.0


def test_v78_model_has_no_output_readout_modules_by_default() -> None:
...
def test_v78_feature_modulation_mode_uses_gate_and_feature_projections() -> None:
    config = ExperimentConfig()
    config.model.cache_output_summary_readout = True
    config.model.cache_output_summary_source = "ambiguity_feature_modulation"
    model = APSGNNModel(config)
    assert model.cache_output_gate_feature_proj is not None
    assert model.cache_output_feature_scale_proj is not None
    assert model.cache_output_feature_shift_proj is not None
    assert model.cache_output_aux_proj is None
...
def test_v78_output_logits_uses_cache_summary_when_enabled() -> None:
    config = ExperimentConfig()
    config.model.cache_output_summary_readout = True
...
def test_v78_feature_modulation_defaults_to_v75_behavior() -> None:
    config = ExperimentConfig()
    config.model.cache_output_summary_readout = True
    config.model.cache_output_summary_source = "ambiguity_feature_modulation"
    config.model.use_reserved_class_slice = False
...
def test_v78_feature_modulation_changes_logits_when_trained() -> None:
    config = ExperimentConfig()
    config.model.cache_output_summary_readout = True
    config.model.cache_output_summary_source = "ambiguity_feature_modulation"
    config.model.cache_output_feature_modulation_scale = 1.5
    ...
    with torch.no_grad():
        ...
        model.cache_output_summary_gate.weight.zero_()
        model.cache_output_summary_gate.bias.fill_(10.0)
        model.cache_output_feature_scale_proj.weight.zero_()
        model.cache_output_feature_scale_proj.bias.fill_(0.4)
        model.cache_output_feature_shift_proj.weight.zero_()
        model.cache_output_feature_shift_proj.bias.copy_(torch.tensor([0.5, -0.5, 0.25, -0.25]))
    residual = torch.zeros(2, config.model.d_model)
...
def test_v78_output_event_carries_gate_features() -> None:
    event = OutputEvent(
...
def test_v78_report_parser_accepts_v78_run_names() -> None:
    module = load_module("build_v78_report", "scripts/build_v78_report.py")
    parsed = module.parse_run_name(
        "20260327-000000-v78-collision-c2-ambigfilm-visit_taskgrad_half_d-32-m-s2234"
    )
    assert parsed is not None
    assert parsed["regime"] == "c2"
    assert parsed["condition"] == "ambigfilm"

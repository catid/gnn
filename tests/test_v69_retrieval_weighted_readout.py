...
def test_v69_generator_emits_retrieval_weighted_readout_pack():
    module = load_module("gen_v69_configs", "scripts/gen_v69_configs.py")
    assert set(module.COLLISION_REGIMES) == {"c1", "c2"}
    assert set(module.CONDITIONS) == {"mean", "retrieved"}
    packs = module.pack_definitions()
    assert packs["experiment"] == "v69_retrieval_weighted_output_readout"


def test_v69_configs_select_output_summary_source_cleanly():
    module = load_module("gen_v69_cfg", "scripts/gen_v69_configs.py")
    mean_cfg = module.build_collision_config("c2", "m", condition="mean")
    retrieved_cfg = module.build_collision_config("c2", "m", condition="retrieved")
    assert mean_cfg["model"]["cache_output_summary_readout"] is True
    assert retrieved_cfg["model"]["cache_output_summary_readout"] is True
    assert mean_cfg["model"]["cache_output_summary_source"] == "mean"
    assert retrieved_cfg["model"]["cache_output_summary_source"] == "retrieved"


def test_v69_model_has_no_output_readout_modules_by_default():
...
def test_v69_select_output_cache_summary_uses_retrieved_source_when_requested():
    config = ExperimentConfig()
    config.model.cache_output_summary_source = "retrieved"
    model = APSGNNModel(config)
...
def test_v69_output_logits_uses_cache_summary_when_enabled():
    config = ExperimentConfig()
...
def test_v69_output_event_carries_cache_summary():
    event = OutputEvent(
...
def test_v69_report_parser_accepts_v69_run_names():
    module = load_module("build_v69_report", "scripts/build_v69_report.py")
    parsed = module.parse_run_name(
        "20260326-230000-v69-collision-c2-retrieved-visit_taskgrad_half_d-32-m-s2234"
    )
    assert parsed is not None
    assert parsed["regime"] == "c2"
    assert parsed["condition"] == "retrieved"

...
def test_v85_generator_emits_ambig_and_intfloor_pack() -> None:
    module = load_module("gen_v85_configs", "scripts/gen_v85_configs.py")
    assert set(module.COLLISION_REGIMES) == {"c1", "c2"}
    assert set(module.CONDITIONS) == {"ambig", "intfloor"}
    packs = module.pack_definitions()
    assert packs["experiment"] == "v85_collision_interaction_floor_gate"


def test_v85_configs_select_sources_cleanly() -> None:
    module = load_module("gen_v85_cfg", "scripts/gen_v85_configs.py")
    ambig_cfg = module.build_collision_config("c2", "m", condition="ambig")
    int_cfg = module.build_collision_config("c2", "m", condition="intfloor")
    assert ambig_cfg["model"]["cache_output_summary_source"] == "ambiguity_gate"
    assert int_cfg["model"]["cache_output_summary_source"] == "collision_interaction_floor_gate"
    assert int_cfg["model"]["cache_output_gate_feature_scale"] == 1.0
    assert int_cfg["model"]["cache_output_gate_floor_scale"] == 1.0
    assert int_cfg["model"]["cache_output_collision_switch_width"] == 2.0


def test_v85_model_has_no_output_readout_modules_by_default() -> None:
    config = ExperimentConfig()
...
def test_v85_intfloor_mode_uses_gate_feature_proj_only() -> None:
    model = APSGNNModel(small_config("collision_interaction_floor_gate"))
...
def test_v85_output_logits_uses_cache_summary_when_enabled() -> None:
...
def test_v85_intfloor_matches_v75_when_interaction_feature_is_zero() -> None:
    ambig_cfg = small_config("ambiguity_gate")
    ambig_cfg.model.use_reserved_class_slice = False
    int_cfg = small_config("collision_interaction_floor_gate")
    int_cfg.model.use_reserved_class_slice = False
    int_cfg.model.cache_output_collision_switch_width = 2.0
    ambig = APSGNNModel(ambig_cfg)
    intfloor = APSGNNModel(int_cfg)
    with torch.no_grad():
        ambig.output_head.weight.zero_()
        ambig.output_head.bias.zero_()
        intfloor.output_head.weight.zero_()
        intfloor.output_head.bias.zero_()
        assert ambig.cache_output_summary_head is not None
        assert intfloor.cache_output_summary_head is not None
        ambig_final = ambig.cache_output_summary_head[-1]
        int_final = intfloor.cache_output_summary_head[-1]
        assert isinstance(ambig_final, torch.nn.Linear)
        assert isinstance(int_final, torch.nn.Linear)
        ambig_final.weight.zero_()
        ambig_final.bias.fill_(1.0)
        int_final.weight.copy_(ambig_final.weight)
        int_final.bias.copy_(ambig_final.bias)
        assert ambig.cache_output_summary_gate is not None
        assert intfloor.cache_output_summary_gate is not None
        ambig.cache_output_summary_gate.weight.zero_()
        ambig.cache_output_summary_gate.bias.zero_()
        intfloor.cache_output_summary_gate.weight.zero_()
        intfloor.cache_output_summary_gate.bias.zero_()
        assert ambig.cache_output_gate_feature_proj is not None
        assert intfloor.cache_output_gate_feature_proj is not None
        ...
        intfloor.cache_output_gate_feature_proj.weight.zero_()
        intfloor.cache_output_gate_feature_proj.bias.zero_()
        intfloor.cache_output_gate_feature_proj.weight[0, 0] = 1.0
    residual = torch.zeros(2, ambig_cfg.model.d_model)
    cache_summary = torch.ones(2, ambig_cfg.model.d_model)
    high_collision = torch.tensor(
        [[0.8, 3.0 / 48.0, 0.0], [0.8, 4.0 / 48.0, 0.0]],
        dtype=residual.dtype,
    )
    ambig_logits = ambig._output_logits(residual, cache_summary=cache_summary, gate_features=high_collision[:, :2])
    int_logits = intfloor._output_logits(residual, cache_summary=cache_summary, gate_features=high_collision)
    assert torch.allclose(ambig_logits, int_logits)


def test_v85_intfloor_adds_confidence_floor_when_collision_is_low() -> None:
    ambig_cfg = small_config("ambiguity_gate")
    ambig_cfg.model.use_reserved_class_slice = False
    int_cfg = small_config("collision_interaction_floor_gate")
    int_cfg.model.use_reserved_class_slice = False
    ...
    residual = torch.zeros(2, ambig_cfg.model.d_model)
    cache_summary = torch.ones(2, ambig_cfg.model.d_model)
    low_collision = torch.tensor(
        [[0.8, 1.0 / 48.0, 0.0], [0.8, 1.5 / 48.0, 0.0]],
        dtype=residual.dtype,
    )
    ambig_logits = ambig._output_logits(residual, cache_summary=cache_summary, gate_features=low_collision[:, :2])
    int_logits = intfloor._output_logits(residual, cache_summary=cache_summary, gate_features=low_collision)
    assert torch.all(int_logits > ambig_logits)


def test_v85_intfloor_interaction_feature_changes_gate_delta() -> None:
    cfg = small_config("collision_interaction_floor_gate")
    ...
    gate_delta = model.cache_output_gate_feature_proj(torch.tensor([[0.4, 4.0 / 48.0, 0.3]]))
    assert float(gate_delta.item()) > 0.0


def test_v85_output_event_carries_three_feature_gate_payload() -> None:
    event = OutputEvent(
...
def test_v85_report_parser_accepts_v85_run_names() -> None:
    module = load_module("build_v85_report", "scripts/build_v85_report.py")
    parsed = module.parse_run_name(
        "20260327-040000-v85-collision-c2-intfloor-visit_taskgrad_half_d-32-m-s2234"
    )
    assert parsed is not None
    assert parsed["regime"] == "c2"
    assert parsed["condition"] == "intfloor"

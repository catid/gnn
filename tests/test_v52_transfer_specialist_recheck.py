...
def test_v52_generator_emits_transfer_specialist_regimes():
    module = load_module("gen_v52_configs", "scripts/gen_v52_configs.py")
    assert set(module.ARMS) == {"visit_taskgrad_half", "stageadaptive_late_half"}
    assert set(module.REGIMES) == {"t1", "t1r", "t2b", "t2c"}
...
def test_v52_eval_completion_filter(tmp_path):
    module = load_module("run_v52_eval_sweep", "scripts/run_v52_eval_sweep.py")
...
def test_v52_report_builder_tracks_four_regimes():
    module = load_module("build_v52_report", "scripts/build_v52_report.py")
    assert set(module.SELECTORS) == {"visit_taskgrad_half", "stageadaptive_late_half"}
    assert set(module.PHASES) == {"t1_xl", "t1r_xl", "t2b_xl", "t2c_xl"}

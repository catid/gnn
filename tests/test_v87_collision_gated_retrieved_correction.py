...
def test_v87_generator_emits_ambig_and_retrhead_pack() -> None:
    module = load_module("gen_v87_configs", "scripts/gen_v87_configs.py")
    assert set(module.COLLISION_REGIMES) == {"c1", "c2"}
    assert set(module.CONDITIONS) == {"ambig", "retrhead"}
    packs = module.pack_definitions()
    assert packs["experiment"] == "v87_collision_gated_retrieved_correction"


def test_v87_configs_select_sources_cleanly() -> None:
    module = load_module("gen_v87_cfg", "scripts/gen_v87_configs.py")
    ambig_cfg = module.build_collision_config("c2", "m", condition="ambig")
    retr_cfg = module.build_collision_config("c2", "m", condition="retrhead")
    assert ambig_cfg["model"]["cache_output_summary_source"] == "ambiguity_gate"
    assert retr_cfg["model"]["cache_output_summary_source"] == "collision_retrieved_head"
    assert retr_cfg["model"]["cache_output_gate_feature_scale"] == 1.0
    assert retr_cfg["model"]["cache_output_collision_switch_width"] == 2.0
    assert retr_cfg["model"]["cache_output_collision_retrieved_scale"] == 1.0


def test_v87_model_has_no_output_readout_modules_by_default() -> None:
    config = ExperimentConfig()
    ...
    assert model.cache_output_collision_retrieved_head is None


def test_v87_retrhead_mode_uses_gate_and_collision_retrieved_head() -> None:
    model = APSGNNModel(small_config("collision_retrieved_head"))
    assert model.cache_output_gate_feature_proj is not None
    assert model.cache_output_gate_feature_proj.in_features == 2
    assert model.cache_output_collision_retrieved_head is not None
...
def test_v87_output_logits_uses_cache_summary_when_enabled() -> None:
...
def test_v87_retrhead_matches_v75_when_retrieved_head_is_zero() -> None:
    ambig_cfg = small_config("ambiguity_gate")
    ambig_cfg.model.use_reserved_class_slice = False
    retr_cfg = small_config("collision_retrieved_head")
    retr_cfg.model.use_reserved_class_slice = False
    ambig = APSGNNModel(ambig_cfg)
    retrhead = APSGNNModel(retr_cfg)
    with torch.no_grad():
        ambig.output_head.weight.zero_()
        ambig.output_head.bias.zero_()
        retrhead.output_head.weight.zero_()
        retrhead.output_head.bias.zero_()
        assert ambig.cache_output_summary_head is not None
        assert retrhead.cache_output_summary_head is not None
        ambig_final = ambig.cache_output_summary_head[-1]
        retr_final = retrhead.cache_output_summary_head[-1]
        assert isinstance(ambig_final, torch.nn.Linear)
        assert isinstance(retr_final, torch.nn.Linear)
        ambig_final.weight.zero_()
        ambig_final.bias.fill_(1.0)
        retr_final.weight.copy_(ambig_final.weight)
        retr_final.bias.copy_(ambig_final.bias)
        assert ambig.cache_output_summary_gate is not None
        assert retrhead.cache_output_summary_gate is not None
        ambig.cache_output_summary_gate.weight.zero_()
        ambig.cache_output_summary_gate.bias.zero_()
        retrhead.cache_output_summary_gate.weight.zero_()
        retrhead.cache_output_summary_gate.bias.zero_()
        assert ambig.cache_output_gate_feature_proj is not None
        assert retrhead.cache_output_gate_feature_proj is not None
        ...
        retrhead.cache_output_gate_feature_proj.weight.zero_()
        retrhead.cache_output_gate_feature_proj.bias.zero_()
        retrhead.cache_output_gate_feature_proj.weight[0, 0] = 1.0
    residual = torch.zeros(2, ambig_cfg.model.d_model)
    cache_summary = torch.ones(2, ambig_cfg.model.d_model)
    retrieved_summary = torch.ones(2, ambig_cfg.model.d_model)
    ...
    retr_logits = retrhead._output_logits(
        residual,
        cache_summary=cache_summary,
        retrieved_summary=retrieved_summary,
        gate_features=gate_features,
    )
    assert torch.allclose(ambig_logits, retr_logits)


def test_v87_retrhead_only_changes_high_collision_outputs() -> None:
    cfg = small_config("collision_retrieved_head")
    ...
    with torch.no_grad():
        ...
        collision_final = model.cache_output_collision_retrieved_head[-1]
    ...
    low_logits = model._output_logits(
        residual,
        cache_summary=cache_summary,
        retrieved_summary=retrieved_summary,
        gate_features=low_collision,
    )
    high_logits = model._output_logits(
        residual,
        cache_summary=cache_summary,
        retrieved_summary=retrieved_summary,
        gate_features=high_collision,
    )
    assert torch.all(high_logits > low_logits)


def test_v87_output_event_carries_two_feature_gate_payload() -> None:
    event = OutputEvent(
...
def test_v87_report_parser_accepts_v87_run_names() -> None:
    module = load_module("build_v87_report", "scripts/build_v87_report.py")
    parsed = module.parse_run_name(
        "20260327-040000-v87-collision-c2-retrhead-visit_taskgrad_half_d-32-m-s2234"
    )
    assert parsed is not None
    assert parsed["regime"] == "c2"
    assert parsed["condition"] == "retrhead"

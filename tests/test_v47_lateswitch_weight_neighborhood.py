...
def test_v47_generator_emits_four_arms():
    module = load_module("gen_v47_configs", "scripts/gen_v47_configs.py")
    assert set(module.ARMS) == {
        "visit_taskgrad_half",
        "stageadaptive_late_quarter",
        "stageadaptive_late_0375",
        "stageadaptive_late_half",
    }
    cfg = module.build_config("stageadaptive_late_0375", "t2a")
    assert cfg["growth"]["adaptive_selector_stage_index_min"] == 5
    assert cfg["growth"]["adaptive_utility_grad_weight"] == 0.375


def test_v47_eval_completion_filter(tmp_path):
    module = load_module("run_v47_eval_sweep", "scripts/run_v47_eval_sweep.py")
...
def test_v47_report_builder_has_all_stage_weights():
    module = load_module("build_v47_report", "scripts/build_v47_report.py")

...
def test_v76_generator_emits_ambig_and_ambigmax_pack() -> None:
    module = load_module("gen_v76_configs", "scripts/gen_v76_configs.py")
    assert set(module.COLLISION_REGIMES) == {"c1", "c2"}
    assert set(module.CONDITIONS) == {"ambig", "ambigmax"}
    packs = module.pack_definitions()
    assert packs["experiment"] == "v76_ambiguity_conditioned_max_correction"


def test_v76_configs_select_sources_cleanly() -> None:
    module = load_module("gen_v76_cfg", "scripts/gen_v76_configs.py")
    ambig_cfg = module.build_collision_config("c2", "m", condition="ambig")
    ambigmax_cfg = module.build_collision_config("c2", "m", condition="ambigmax")
    assert ambig_cfg["model"]["cache_output_summary_source"] == "ambiguity_gate"
    assert ambigmax_cfg["model"]["cache_output_summary_source"] == "ambiguity_max"
    assert ambigmax_cfg["model"]["cache_output_gate_feature_scale"] == 1.0
    assert ambigmax_cfg["model"]["cache_output_max_scale"] == 1.0


def test_v76_model_has_no_output_readout_modules_by_default() -> None:
...
def test_v76_ambigmax_mode_uses_gate_and_aux_projections() -> None:
    config = ExperimentConfig()
    config.model.cache_output_summary_readout = True
    config.model.cache_output_summary_source = "ambiguity_max"
    model = APSGNNModel(config)
...
def test_v76_output_logits_uses_cache_summary_when_enabled() -> None:
    config = ExperimentConfig()
    config.model.cache_output_summary_readout = True
...
def test_v76_ambigmax_defaults_to_v75_behavior() -> None:
    config = ExperimentConfig()
    config.model.cache_output_summary_readout = True
    config.model.cache_output_summary_source = "ambiguity_max"
    config.model.use_reserved_class_slice = False
...
def test_v76_max_correction_changes_logits_when_trained() -> None:
    config = ExperimentConfig()
    config.model.cache_output_summary_readout = True
    config.model.cache_output_summary_source = "ambiguity_max"
    config.model.cache_output_max_scale = 2.0
    ...
    shifted = model._output_logits(residual, cache_summary=cache_summary, aux_summary=aux_summary)
    assert torch.all(shifted > base)


def test_v76_output_event_carries_gate_features() -> None:
    event = OutputEvent(
...
def test_v76_report_parser_accepts_v76_run_names() -> None:
    module = load_module("build_v76_report", "scripts/build_v76_report.py")
    parsed = module.parse_run_name(
        "20260326-230000-v76-collision-c2-ambigmax-visit_taskgrad_half_d-32-m-s2234"
    )
    assert parsed is not None
    assert parsed["regime"] == "c2"
    assert parsed["condition"] == "ambigmax"

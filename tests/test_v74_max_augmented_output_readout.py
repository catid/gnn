...
def test_v74_generator_emits_maxaug_pack() -> None:
    module = load_module("gen_v74_configs", "scripts/gen_v74_configs.py")
    assert set(module.COLLISION_REGIMES) == {"c1", "c2"}
    assert set(module.CONDITIONS) == {"mean", "maxaug"}
    packs = module.pack_definitions()
    assert packs["experiment"] == "v74_max_augmented_output_readout"


def test_v74_configs_select_maxaug_source_cleanly() -> None:
    module = load_module("gen_v74_cfg", "scripts/gen_v74_configs.py")
    mean_cfg = module.build_collision_config("c2", "m", condition="mean")
    maxaug_cfg = module.build_collision_config("c2", "m", condition="maxaug")
    assert mean_cfg["model"]["cache_output_summary_readout"] is True
    assert maxaug_cfg["model"]["cache_output_summary_readout"] is True
    assert mean_cfg["model"]["cache_output_summary_source"] == "mean"
    assert maxaug_cfg["model"]["cache_output_summary_source"] == "max_augmented"
    assert maxaug_cfg["model"]["cache_output_max_scale"] == 1.0


def test_v74_model_has_no_output_readout_modules_by_default() -> None:
...
def test_v74_maxaug_mode_uses_aux_proj_only() -> None:
    config = ExperimentConfig()
    config.model.cache_output_summary_readout = True
    config.model.cache_output_summary_source = "max_augmented"
    model = APSGNNModel(config)
...
def test_v74_output_logits_uses_cache_summary_when_enabled() -> None:
    config = ExperimentConfig()
    config.model.cache_output_summary_readout = True
...
def test_v74_max_augmentation_defaults_to_identity() -> None:
    config = ExperimentConfig()
    config.model.cache_output_summary_readout = True
    config.model.cache_output_summary_source = "max_augmented"
    config.model.d_model = 8
...
def test_v74_max_augmentation_adds_learned_correction() -> None:
    config = ExperimentConfig()
    config.model.cache_output_summary_readout = True
    config.model.cache_output_summary_source = "max_augmented"
...
def test_v74_output_event_carries_all_summaries() -> None:
    event = OutputEvent(
...
def test_v74_report_parser_accepts_v74_run_names() -> None:
    module = load_module("build_v74_report", "scripts/build_v74_report.py")
    parsed = module.parse_run_name(
        "20260326-230000-v74-collision-c2-maxaug-visit_taskgrad_half_d-32-m-s2234"
    )
    assert parsed is not None
    assert parsed["regime"] == "c2"
    assert parsed["condition"] == "maxaug"

...
def test_v57_generator_emits_all_rule_regimes():
    module = load_module("gen_v57_configs", "scripts/gen_v57_configs.py")
...
def test_v57_eval_completion_filter(tmp_path):
    module = load_module("run_v57_eval_sweep", "scripts/run_v57_eval_sweep.py")
...
def test_v57_report_builder_tracks_rules_and_scopes():
    module = load_module("build_v57_report", "scripts/build_v57_report.py")
    assert set(module.SELECTORS) == {"visit_taskgrad_half", "stageadaptive_late_half"}
    assert set(module.RULES) == {"single_selector_vt_half", "regime_keyed_rule"}
